...
export type Vehicle = { id: string; registration: string; fleetNumber?: string; abbreviation?: string; transmission?: string; dvsCompliant?: boolean; fuelProvider?: string; cabMobile?: string; fuelPin?: string; shellCard?: string; bpRedCard?: string; bpPlainCard?: string; notes?: string; fuelPinSecretName?: string; fuelCardLastFour?: string; fleetioId?: string; fleetioName?: string; fleetioStatus?: string; fleetioVor?: boolean; fleetioPmiDueUtc?: string; fleetioMotDueUtc?: string; fleetioServiceStatus?: string; fleetioLastSyncedUtc?: string; active: boolean };
export type Driver = { id: string; employeeNumber: string; displayName: string; tachoName?: string; mobileNumber?: string; driverType?: string; driverGroup?: string; skills?: string; coding?: string; agencyName?: string; northEligible?: boolean; preloadEligible?: boolean; notes?: string; tachoMasterDriverId?: string; drivingLicenceNumber?: string; licenceExpiry?: string; licenceStatus?: string; lastTachoSyncUtc?: string; active: boolean };
...
export type Site = { id: string; externalCode: string; name: string; driverTextName?: string; aliases?: string; collectionAddress?: string; collectionInstructions?: string; mapLink?: string; customField1?: string; customField2?: string; customField3?: string; active: boolean };
...
export type TachoVehicleDriverStatus = { memberCode: number; driverName: string; cardNumber?: string; employeeNumber?: string; dutyStartUtc: string; dutyEndUtc?: string; workMinutes: number; restMinutes: number; availableMinutes: number; driveMinutes: number; breakCount: number; breakMinutes?: number; metricsValidAtUtc?: string; dailyDriverPeriodsAvailable?: number; driveAvailableTodayMinutes?: number; driveAvailableTomorrowMinutes?: number; driveAvailableWeekMinutes?: number; driveAvailableFortnightMinutes?: number; longDaysWorkedThisWeek?: number; shortDailyRestTakenThisWeek?: number; workAvailableWeekMinutes?: number };
export type FleetStatus = { provider: string; retrievedAtUtc: string; vehicleCount: number; readyCount: number; attentionCount: number; vehicles: Array<{ vehicleId: string; registration: string; fleetNumber?: string; trackingIdentifier?: string; condition: 'Moving' | 'Started' | 'Parked' | 'Stationary' | 'SignedOn' | 'Stale' | 'NotSignedOn'; lastEventTimeUtc?: string; ignitionOn?: boolean; isMoving?: boolean; speedKph?: number; latitude?: number; longitude?: number; ageMinutes?: number; loadId?: string; loadReference?: string; loadStatus?: string; driverId?: string; driverName?: string; tachoName?: string; driverSource?: 'TachoMaster' | 'Allocation'; allocatedDriverName?: string; driverMismatch?: boolean; plannedDutyUtc?: string; tacho?: TachoVehicleDriverStatus; fleetioId?: string; fleetioName?: string; fleetioStatus?: string; fleetioVor?: boolean; fleetioPmiDueUtc?: string; fleetioMotDueUtc?: string; fleetioServiceStatus?: string }> };
...
export type MasterApplyResponse = { received: number; applied: number; registered?: number; failed: number; linked?: number; results: Array<{ entityType: string; idempotencyKey?: string; applied: boolean; registered?: boolean; error?: string }> };
...
export type TachoMasterStatus = { configured: boolean; connected: boolean; matchedVehicleCount: number; missingSettings?: string[]; message: string };
...
export const api = {
  ...
  updateDriver: (id: string, payload: Omit<Driver, 'id' | 'lastTachoSyncUtc'>, token?: string) => request<Driver>(`/api/v1/drivers/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  trailers: (token?: string) => request<Trailer[]>('/api/v1/trailers', token),
  updateTrailer: (id: string, payload: Omit<Trailer, 'id'>, token?: string) => request<Trailer>(`/api/v1/trailers/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  sites: (token?: string) => request<Site[]>('/api/v1/sites', token),
  updateSite: (id: string, payload: Omit<Site, 'id'>, token?: string) => request<Site>(`/api/v1/sites/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  ...
  tachoMasterStatus: (token?: string) => request<TachoMasterStatus>('/api/v1/integrations/tachomaster/status', token),
