...
export type MasterApplyResponse = { [key: string]: any; received: number; applied: number; failed: number; linked?: number; registered?: number; results: Array<{ entityType: string; idempotencyKey?: string; applied: boolean; registered?: boolean; error?: string }> };
...
export const api: Record<string, any> = {
  ...
  marketContacts: async (token?: string) => normaliseMarketContacts(await request<MarketContact[]>('/api/v1/market-contacts', token)),
  ...
  stageBatch: (records: StageBatchRequest[], token?: string) => request<StageBatchResponse>('/api/v1/staging/batch', token, { method: 'POST', body: JSON.stringify(normaliseMarketMasterRecords(records)) }),
  applyMasterData: (records: StageBatchRequest[], token?: string) => request<MasterApplyResponse>('/api/v1/master-data/apply', token, { method: 'POST', body: JSON.stringify(normaliseMarketMasterRecords(records)) }),
  ...
  syncFleetioVehicles: (token?: string) => request<FleetioSync>('/api/v1/integrations/fleetio/sync-assets-resilient', token, { method: 'POST' }),
