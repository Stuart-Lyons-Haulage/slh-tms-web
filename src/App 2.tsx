...
import { Dashboard, LiveTracking, MasterData, OperationalPlaceholder, StagingQueue } from './pages/Pages';
...
function Shell() {
  ...
  return <div className="app-shell"><header><button className="menu" onClick={() => setOpen(!open)} aria-label="Toggle navigation">☰</button><div className="brand"><span>SLH</span><small>Transport Management</small></div><div className="header-actions">{authenticated ? <><span className="user">{accounts[0]?.name}</span><button onClick={() => instance.logoutRedirect()}>Sign out</button></> : <button className="primary" onClick={signIn} disabled={!apiScope}>Sign in with Microsoft</button>}</div></header><aside className={open ? 'open' : ''}><div className="nav-title">Control centre</div>{navigation.map(([path, label]) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setOpen(false)}>{label}</NavLink>)}</aside><main>{!authenticated && <div className="notice">Connect Microsoft Entra ID to use live TMS data. Configure the values in `.env.local`.</div>}<Routes><Route path="/" element={<Dashboard />} /><Route path="/tracking" element={<LiveTracking />} /><Route path="/staging" element={<StagingQueue />} /><Route path="/master-data" element={<MasterData />} />{navigation.slice(1).filter(([path]) => !['/tracking', '/staging', '/master-data'].includes(path)).map(([path, label]) => <Route key={path} path={path} element={<OperationalPlaceholder title={label} />} />)}</Routes></main></div>;
}
