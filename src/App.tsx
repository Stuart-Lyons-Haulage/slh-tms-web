...
function Shell() {
  ...
  return <div className={`app-shell ${authenticated ? 'with-system-strip' : ''}`}>
    ...
    {authenticated && open && <button className="mobile-nav-scrim" type="button" aria-label="Close navigation" onClick={closeMobile} />}
    <aside className={`side-nav ${open ? 'open' : ''}`}>
      {authenticated && <div className="mobile-nav-search"><GlobalSearch /></div>}
      <NavLink className="new-order" to="/planner-import" onClick={closeMobile}>＋ Import</NavLink>
    ...
    {authenticated && <><TmsAssistant /><MobileDock openMenu={() => setOpen(true)} /></>}
  </div>;
}
