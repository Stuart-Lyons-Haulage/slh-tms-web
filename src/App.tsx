...
import { Admin, CustomersMaster, Dashboard, DriverAssignments, DriversMaster, Exceptions, ExportCentre, FleetAssetsMaster, FuelMaster, LiveTracking, Loads, MarketsMaster, MasterData, Orders, PlanningBoard, Reporting, SitesMaster, StagingQueue } from './pages/Pages';
import { apiScope } from './lib/auth';
import { TmsAssistant } from './components/TmsAssistant';

const navigation = [['/', 'Planner'], ['/dashboard', 'Operations dashboard'], ['/order-intake', 'Order entry'], ['/loads', 'Loads'], ['/allocation', 'Allocation'], ['/driver-assignments', 'Driver assignments'], ['/tracking', 'Live tracking'], ['/staging', 'Staging review'], ['/exceptions', 'Exceptions'], ['/customers', 'Customers'], ['/drivers', 'Drivers'], ['/fleet-assets', 'Vehicles + fuel / trailers'], ['/markets', 'Markets'], ['/fuel', 'Fuel'], ['/sites', 'Sites & contacts'], ['/master-data', 'Master data overview'], ['/reporting', 'Reporting'], ['/exports', 'Exports'], ['/admin', 'Admin']];
