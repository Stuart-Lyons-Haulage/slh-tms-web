...
type RunProgressResponse = { planningDate: string; calculatedAtUtc: string; count: number; records: RunProgressRecord[] };
...
export function PilotRunHealth() {
  ...
  const load = useCallback(async () => { const token = await getToken(); return request<RunProgressResponse>("/api/v1/run-progress", token); }, [getToken]);
  const { data, error, loading, refresh } = useApi(load);

  const captureEta = useCallback(async () => {
    try {
      const token = await getToken();
      const etas = await request<DeliveryEtaResponse>(`/api/v1/operations/delivery-etas?date=${encodeURIComponent(localDate())}`, token, undefined, 40000);
  ...
  return <section className="panel pilot-health-panel">
    ...
    {!error && data && <p className="hint">Planning date {data.planningDate} · last calculated {new Date(data.calculatedAtUtc).toLocaleString("en-GB")} · {etaCapture}</p>}
