  ...
  return (
    <div className="runs-operational-page">
      ...
      <section className="panel run-lock-control">
        <div className="title-row">
          <div><p className="eyebrow">Operational baseline</p><h2>Lock plan</h2><p className="hint">Lock the run plan here once allocations and routes are ready. Later changes remain possible and are audited in Plan Stability.</p></div>
          <label>Planning date <input type="date" value={lockDate} onChange={(event) => { setLockDate(event.target.value); setLockMessage(undefined); }} disabled={locking} /></label>
        </div>
        {readiness.error && <p className="notice inline-notice">Plan status could not refresh: {readiness.error}</p>}
        {readiness.data && <div className="run-lock-status">
          <div><strong>{readiness.data.planLock ? "✓ PLAN LOCKED" : "PLAN OPEN"}</strong><span>{readiness.data.runs} runs · {readiness.data.missingAllocations} missing allocation{readiness.data.missingAllocations === 1 ? "" : "s"}</span>{readiness.data.planLock && <small>Locked {formatDateTime(readiness.data.planLock.lockedAtUtc)} · {readiness.data.planLock.baselineRuns} baseline runs</small>}</div>
          {!readiness.data.planLock && <button className="primary" type="button" onClick={() => void lockPlan()} disabled={locking || readiness.loading || readiness.data.runs === 0}>{locking ? "Locking…" : "Lock plan"}</button>}
        </div>}
