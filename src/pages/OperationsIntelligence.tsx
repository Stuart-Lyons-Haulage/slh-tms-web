...
import { addDaysIso, formatDate, formatDateLong, formatDateTime, todayIsoDate } from '../lib/dateUtils';

const daysAgo = (days: number) => addDaysIso(-days);
...
export function MorningReadiness() {
  const token = useAccessToken(); const [date, setDate] = useState(todayIsoDate());
  const report = useApi(useCallback(async () => intelligenceApi.readiness(date, await token()), [date, token]));
  const lock = async () => { if (!window.confirm(`Lock the operational baseline for ${formatDate(date)}? Changes will remain possible but require a reason.`)) return; await intelligenceApi.lockPlan(date, await token()); await report.refresh(); };
  return <section className="intel-page"><div className="intel-heading"><div><p className="eyebrow">Pre-operation gate</p><h1>Morning Readiness</h1><p>Is the plan ready to operate?</p></div><label>Planning date<input type="date" value={date} onChange={e => setDate(e.target.value)} /></label></div>
    ...
    {report.data && <><div className={`readiness-banner ${report.data.ready ? 'ready' : 'not-ready'}`}><strong>{report.data.ready ? '✓ READY TO OPERATE' : '⚠ ACTION REQUIRED'}</strong><span>{report.data.runs} runs planned for {formatDateLong(date)}</span></div>
      ...
      <section className="lock-panel"><div><p className="eyebrow">Plan baseline</p><h2>{report.data.planLock ? `Locked ${formatDateTime(report.data.planLock.lockedAtUtc)}` : 'Plan is still editable without change reasons'}</h2><p>{report.data.planLock ? `${report.data.planLock.baselineRuns} baseline runs. Any driver, vehicle, route or status change now requires a reason and is counted in Plan Stability.` : 'Lock the plan once the planner is satisfied. Emergency changes are still allowed and fully audited.'}</p></div>{!report.data.planLock && <button className="primary" onClick={lock}>Lock Plan</button>}</section></>}
  </section>;
}
...
export function TimelinePage({ kind }: { kind: 'run' | 'order' }) {
  ...
  return <section className="intel-page"><div className="intel-heading"><div><p className="eyebrow">Audit timeline</p><h1>{report.data?.reference ?? (kind === 'run' ? 'Run timeline' : 'Order timeline')}</h1><p>{report.data ? `${report.data.entityType} · ${report.data.status}${report.data.planningDate ? ` · ${formatDate(report.data.planningDate)}` : ''}` : 'Loading complete operational history…'}</p></div><NavLink className="button-link" to={kind==='run'?'/loads':'/order-intake'}>Back to {kind==='run'?'Runs':'Orders'}</NavLink></div>
    {report.loading && <div className="state">Loading timeline…</div>}{report.error && <div className="state error">{report.error}</div>}
    {report.data && <div className="timeline">{report.data.events.length === 0 ? <div className="state">No timeline events recorded.</div> : report.data.events.map((event,index) => <article key={`${event.atUtc}-${index}`}><div className="timeline-dot"/><div className="timeline-time">{formatDateTime(event.atUtc)}</div><div className="timeline-card"><span>{event.source}</span><h2>{event.title}</h2><p>{event.detail}</p>{event.by && <small>By {event.by}</small>}</div></article>)}</div>}
  </section>;
}

export function PlanStability() {
  const token = useAccessToken(); const [from,setFrom] = useState(daysAgo(29)); const [to,setTo] = useState(todayIsoDate());
