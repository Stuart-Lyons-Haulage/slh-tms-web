  ...
  return <div className="ops-control-clean">
    <section className="panel" style={{ marginBottom: 18 }}>
      ...
      <div className="admin-grid">
        <StatusCard title="Sage HR" configured={sage.data?.configured} connected={sage.data?.connected} detail={sage.data?.message || sage.error || "Checking Sage HR…"} meta={sage.data ? `${sage.data.employeeCount} employees · ${sage.data.driverCandidateCount} driver candidates` : undefined} />
        <StatusCard title="TachoMaster" configured={tacho.data?.configured} connected={tacho.data?.connected} detail={tacho.data?.message || tacho.error || "Checking TachoMaster…"} meta={tacho.data ? `${tacho.data.matchedVehicleCount} current vehicle/driver assignments` : undefined} />
        <StatusCard title="DOT / RoadTech" configured={road.data?.configured} connected={road.data?.connected} detail={road.data?.message || road.error || "Checking DOT / RoadTech…"} meta={road.data ? `${road.data.recordCount} live vehicle records` : undefined} />
    ...
    <style>{`.ops-control-clean > section:last-child .admin-grid > .admin-card:first-child > small:first-of-type{display:none!important}`}</style>
    <OperationsControl />
  </div>;
}
