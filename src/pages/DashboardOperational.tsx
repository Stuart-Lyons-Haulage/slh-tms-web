...
export function DashboardOperational() {
  ...
  const refreshAll = useCallback(() => Promise.allSettled([refreshReadiness(), refreshAttention(), refreshFreshness()]).then(() => undefined), [refreshAttention, refreshFreshness, refreshReadiness]);

  useEffect(() => startVisiblePolling(refreshAll, 60_000), [refreshAll]);

  return <section className="dashboard-health-page">
    <div className="title-row dashboard-health-title"><div><p className="eyebrow">Operational health · {formatDateLong(date)}</p><h1>Today at a glance</h1><p className="hint">A decision-focused view of today's loads, runs, people, fleet, exceptions and the systems feeding the operation.</p></div><button type="button" onClick={() => void refreshAll()} disabled={readiness.loading || attention.loading || freshness.loading}>Refresh all</button></div>
    {readiness.error && <p className="notice inline-notice">Operational health could not refresh: {readiness.error}</p>}
    {snapshot && <><div className={`dashboard-health-state ${snapshot.ready ? "good" : "attention"}`}><div><span>{snapshot.ready ? "✓" : "!"}</span><div><small>Operational health</small><strong>{snapshot.ready ? "READY TO OPERATE" : "ACTION REQUIRED"}</strong></div></div><p>{snapshot.runs} runs today · {readyRuns} fully allocated · {attention.data?.count || 0} active exception{attention.data?.count === 1 ? "" : "s"}</p></div><div className="dashboard-health-grid"><Link to="/staging"><article className={snapshot.unreviewedOrders ? "attention" : "good"}><span>Loads waiting</span><strong>{snapshot.unreviewedOrders}</strong><small>Need review / approval</small></article></Link><Link to="/driver-dispatch"><article className={snapshot.missingAllocations ? "attention" : "good"}><span>Runs ready</span><strong>{readyRuns}/{snapshot.runs}</strong><small>{snapshot.missingAllocations} need allocation</small></article></Link><Link to="/fleet-assets"><article className={snapshot.vorConflicts ? "attention" : "good"}><span>Fleet / VOR</span><strong>{snapshot.vorConflicts}</strong><small>Conflicts against today's plan</small></article></Link><Link to="/night-outs"><article className={snapshot.tachoConcerns ? "attention" : "good"}><span>Driver compliance</span><strong>{snapshot.tachoConcerns}</strong><small>Hours / Tacho concerns</small></article></Link><Link to="/attention"><article className={highAttention ? "attention" : "good"}><span>High priority</span><strong>{highAttention}</strong><small>{attention.data?.count || 0} total exceptions</small></article></Link><Link to="/driver-dispatch"><article className={snapshot.planLock ? "good" : "neutral"}><span>Plan baseline</span><strong>{snapshot.planLock ? "Locked" : "Open"}</strong><small>{snapshot.planLock ? `${snapshot.planLock.baselineRuns} baseline runs` : "Not locked yet"}</small></article></Link></div></>}
