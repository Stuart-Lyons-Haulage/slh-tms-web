...
export function DashboardOperational() {
  ...
  return <section className="dashboard-health-page">
    ...
    <div className="dashboard-refresh-strip" role="group" aria-label="Refresh dashboard panels">
      ...
      <button type="button" onClick={() => void refreshCore()} disabled={readiness.loading || attention.loading || freshness.loading}>Health and exceptions</button>
      <button type="button" onClick={() => void refreshLiveCompliance()} disabled={liveFleet.loading || compliance.loading}>Fleet and compliance</button>
    </div>
