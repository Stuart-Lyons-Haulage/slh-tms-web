...
/**
 * The TV live-runs endpoint remains authoritative for which rows are visible.
 * This helper only replaces the fallback ETA on an active row with the richer
 * cumulative final ETA calculated by /run-timing. Completed rows are never
 * reintroduced and an ARRIVED timestamp always wins over a calculated ETA.
 */
