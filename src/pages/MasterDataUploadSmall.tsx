...
function toFlatRecord(value: unknown, label: string): JsonRecord {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    throw new Error(`${label}.${key} must be a string, number or boolean. Nested objects and arrays are not accepted.`);
  }
  return result;
}
...
function parseUploadJson(text: string, fileName: string): ParsedUpload {
  ...
  const grouped = new Map<SupportedEntity, unknown[]>();
  supportedEntities.forEach((entity) => grouped.set(entity, []));
  const warnings: string[] = [];

  if (Array.isArray(json)) {
    json.forEach((item, index) => {
      if (!isPlainObject(item)) throw new Error(`Row ${index + 1} must be a JSON object.`);
      const entity = normaliseEntity(item.entityType ?? item.type);
      if (!entity) throw new Error(`Row ${index + 1} needs entityType set to driver, site, vehicle or trailer.`);
      const payload = isPlainObject(item.payload) ? item.payload : Object.fromEntries(Object.entries(item).filter(([key]) => key !== 'entityType' && key !== 'type'));
      grouped.get(entity)!.push(payload);
    });
    ...
    for (const entity of supportedEntities) {
      ...
      grouped.set(entity, value);
    }
    if (!found) {
      throw new Error('Use a JSON object containing drivers, sites, vehicles and/or trailers arrays, or an array where each row has entityType.');
    }
  } else {
  ...
  const counts = { driver: 0, site: 0, vehicle: 0, trailer: 0 };
  for (const entity of supportedEntities) {
    const rows = grouped.get(entity) || [];
    rows.forEach((row, index) => {
      const payload = toFlatRecord(row, `${pluralKeys[entity]}[${index}]`);
      const identity = recordIdentity(entity, payload, index);
      if (identity === `${entity}-${index + 1}`) warnings.push(`${pluralKeys[entity]} row ${index + 1} has no obvious identity field; it will still be sent for API validation.`);
      records.push({
        entityType: entity,
        idempotencyKey: `uploadsmall:${entity}:${identity}`.toLowerCase().replace(/[^a-z0-9:_-]+/g, '-'),
        source: `UploadSmall JSON · ${fileName}`,
        payload,
      });
      counts[entity] += 1;
    });
...
export function MasterDataUploadSmall({ onApplied }: { onApplied: () => void }) {
  ...
  return <div className="panel" style={{ marginBottom: 18, border: '2px solid #d5e0e4' }}>
    ...
    {open && <div style={{ display: 'grid', gap: 12 }}>
      ...
      <p className="hint">Accepted shape: <code>{'{ "drivers": [...], "sites": [...], "vehicles": [...], "trailers": [...] }'}</code>. You can include any subset of those four arrays.</p>
