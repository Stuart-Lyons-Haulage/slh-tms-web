...
export function GeofenceOperational() {
  ...
  const rows = useMemo(() => {
    ...
    return (data?.records || []).filter(row => (includeArchived || row.active) && (!q || [row.name, row.siteNumber, row.category, row.validationStatus].some(value => String(value || '').toLowerCase().includes(q))));
  ...
  return <section>
    ...
    {selected && <div className="panel" style={{ marginBottom: 16 }}>
      ...
      <div className="form-grid">
        ...
        <label>Site number<input value={String(draft.siteNumber || '')} onChange={e => setDraft(v => ({ ...v, siteNumber: e.target.value }))}/></label>
      ...
      <div className="actions"><button className="primary" disabled={saving} onClick={() => void save()}>Save geofence</button><button onClick={() => setSelected(undefined)}>Cancel</button></div>
    </div>}
    ...
    <div className="panel">
      ...
      {loading && !data ? <div className="state">Loading repaired geofence data…</div> : <div style={{ overflowX: 'auto' }}><table><thead><tr><th>Geofence</th><th>Site no.</th><th>Category</th><th>Polygon</th><th>Site link</th><th>Entry confirm</th><th>Max wait</th><th>Status</th><th>Actions</th></tr></thead><tbody>{rows.map(row => <tr key={row.id}><td><strong>{row.name}</strong></td><td>{text(row.siteNumber)}</td><td>{text(row.category)}</td><td>{row.polygonValid ? 'Valid' : 'Invalid'}</td><td>{row.siteLinked ? 'Linked' : 'Unlinked'}</td><td>{row.pendingEntryMinutes} min</td><td>{row.maxWaitMinutes == null ? '—' : `${row.maxWaitMinutes} min`}</td><td>{row.active ? row.validationStatus : 'Archived'}</td><td style={{ whiteSpace: 'nowrap' }}><button onClick={() => { setSelected(row); setDraft({ ...row }); }}>Edit</button>{' '}<button disabled={saving} onClick={() => void setActive(row, !row.active)}>{row.active ? 'Archive' : 'Restore'}</button>{!row.active && <>{' '}<button disabled={saving} onClick={() => void deleteFence(row)} style={{ borderColor: '#b42318', color: '#b42318' }}>Delete</button></>}</td></tr>)}</tbody></table>{!rows.length && <div className="state">No geofences match this filter.</div>}</div>}
    </div>
  </section>;
}
