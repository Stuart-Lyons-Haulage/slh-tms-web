...
export function PalletPlanningControl() {
  ...
  useEffect(() => {
    const id = window.setInterval(() => void refreshControl(), 5000);
  ...
  return <section>
    <div className="title-row"><div><p className="eyebrow">Planner second screen · live quantity control</p><h1>Pallet control</h1><p className="intro">The TMS equivalent of the Pallet Order sheet. Collection rows and delivery columns are created only from live orders for the selected date; planned quantities then reduce the outstanding boxes as runs are built.</p></div><div className="title-actions"><label>Planning date <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setSelectedCell(undefined); }} /></label><button onClick={() => void refreshControl()} disabled={control.loading}>Refresh</button><Link className="button-like primary" to="/">Open planner</Link></div></div>
    ...
    {data && <>
      ...
      <div className="master-table-wrap" style={{ overflow: "auto", maxHeight: "65vh", border: "1px solid #d5e0e4", borderRadius: 8 }}><table className="master-table" style={{ minWidth: Math.max(1050, 210 + data.destinations.length * 92), borderCollapse: "separate", borderSpacing: 0 }}><thead><tr><th style={{ position: "sticky", left: 0, zIndex: 4, minWidth: 210, background: "#eef4f5" }}>Collection site</th>{data.destinations.map((destination) => <th key={destination} style={{ minWidth: 88, writingMode: "vertical-rl", transform: "rotate(180deg)", height: 150 }}>{destination}</th>)}<th style={{ minWidth: 90 }}>Total</th></tr></thead><tbody>
        {data.planningGroups.map((group) => { const groupCells = data.destinations.map((destination) => cellMap.get(`${group}|||${destination}`)); const rowTotal = groupCells.reduce((sum, cell) => sum + metric(cell), 0); if (mode === "outstanding" && rowTotal === 0 && !groupCells.some((cell) => (cell?.overplanned || 0) > 0)) return null; return <tr key={group}><td style={{ position: "sticky", left: 0, zIndex: 2, background: "white", fontWeight: 700 }}>{group}</td>{data.destinations.map((destination, index) => { const cell = groupCells[index]; const number = metric(cell); const over = cell?.overplanned || 0; return <td key={destination} style={{ textAlign: "center", padding: 3 }}>{showCell(cell) && (number > 0 || over > 0) ? <button type="button" onClick={() => setSelectedCell({ group, destination })} title={`${group} → ${destination}: ${cell?.ordered || 0} ordered, ${cell?.planned || 0} planned, ${cell?.outstanding || 0} outstanding`} style={{ width: "100%", minHeight: 38, fontWeight: 800, border: over > 0 ? "2px solid #b42318" : number === 0 ? "1px solid #b7c6cc" : undefined, background: over > 0 ? "#fff1f0" : mode === "outstanding" ? "#fff8e9" : undefined }}>{number}{over > 0 && <small style={{ display: "block", color: "#b42318" }}>+{over}</small>}</button> : ""}</td>; })}<td style={{ textAlign: "center", fontWeight: 800 }}>{rowTotal}</td></tr>; })}
        <tr><td style={{ position: "sticky", left: 0, background: "#eef4f5", fontWeight: 800 }}>Destination total</td>{data.destinations.map((destination) => { const total = data.planningGroups.reduce((sum, group) => sum + metric(cellMap.get(`${group}|||${destination}`)), 0); return <td key={destination} style={{ textAlign: "center", fontWeight: 800 }}>{total || ""}</td>; })}<td style={{ textAlign: "center", fontWeight: 900 }}>{mode === "ordered" ? data.summary.ordered : mode === "planned" ? data.summary.planned : data.summary.outstanding}</td></tr>
      </tbody></table></div>
