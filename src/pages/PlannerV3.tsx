...
const PERIOD_ORDER: Record<PlanningPeriod, number> = { AM: 0, PM: 1, TBC: 2 };
...
function periodFromTime(time: string): PlanningPeriod {
...
function runPeriod(load: Load): PlanningPeriod {
  const notePeriod = tagged(load.plannerNotes, "Planner period").toUpperCase();
  if (notePeriod === "AM" || notePeriod === "PM") return notePeriod;
  const firstTimedStop = safeStops(load)
    .map((stop) => text(stop.plannedArrivalUtc))
    .find(Boolean);
  if (firstTimedStop) {
    const parsed = new Date(firstTimedStop);
    if (!Number.isNaN(parsed.getTime())) return parsed.getHours() < 12 ? "AM" : "PM";
  }
  return "TBC";
}
...
class PlannerBoundary extends Component<{ children: ReactNode }, { error?: Error }> {
  state: { error?: Error } = {};
  static getDerivedStateFromError(error: Error) { return { error }; }
  componentDidCatch(error: Error, info: ErrorInfo) { console.error("Planner V3 isolated error", error, info); }
  render() {
    if (!this.state.error) return this.props.children;
    return <section className="operational-planner">
      <div className="panel" style={{ padding: 24 }}>
        <p className="eyebrow">Planner recovery</p>
        <h1>The Planner caught an internal error</h1>
        <p>You are still signed in and the rest of the TMS has not been replaced by the error screen.</p>
        <pre style={{ whiteSpace: "pre-wrap" }}>{this.state.error.name}: {this.state.error.message}</pre>
        <button className="primary" onClick={() => this.setState({ error: undefined })}>Retry Planner view</button>
      </div>
    </section>;
  }
}
...
class MapBoundary extends Component<{ children: ReactNode }, { error?: Error }> {
  state: { error?: Error } = {};
  static getDerivedStateFromError(error: Error) { return { error }; }
  componentDidCatch(error: Error, info: ErrorInfo) { console.error("Planner V3 map isolated error", error, info); }
  render() {
    if (!this.state.error) return this.props.children;
    return <div className="op-map-empty"><div style={{ padding: 24 }}>
      <strong>Map renderer failed safely</strong>
      <p>{this.state.error.message}</p>
      <small>The job and run controls remain available.</small>
    </div></div>;
  }
}
...
function PlannerMap({
  ...
  const points = useMemo<MapPoint[]>(() => {
    ...
    for (const item of jobs) {
      ...
      if (delivery) result.push({
        ...
        kind: item.order.id === selectedJob?.order.id ? "selected" : "delivery",
      });
    }
    ...
    return result;
  }, [geocodes, jobs, selectedJob, selectedLoad, sites]);

  const fallbackQuery = selectedJob?.deliveryAddress || selectedJob?.destination || selectedJob?.collectionSite || "United Kingdom";

  useEffect(() => {
    if (!container.current || !mapsClientId || !appClientId || !tenantId) return;
    let disposed = false;
    const timeout = window.setTimeout(() => {
      if (!disposed && !ready) setError("Azure Maps did not finish loading within 8 seconds.");
    }, 8000);
    try {
      ...
      map.events.add("ready", () => {
        ...
        try {
          ...
          map.layers.add(new atlas.layer.LineLayer(source, "planner-v3-route", {
            strokeColor: "#007979",
            strokeWidth: 5,
            filter: ["==", ["get", "kind"], "route"],
          }));
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v3-delivery", {
            color: "#e3912e", radius: 8, strokeColor: "#fff", strokeWidth: 2,
            filter: ["==", ["get", "kind"], "delivery"],
          }));
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v3-collection", {
            color: "#315f86", radius: 9, strokeColor: "#fff", strokeWidth: 2,
            filter: ["==", ["get", "kind"], "collection"],
          }));
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v3-run", {
            color: "#007979", radius: 10, strokeColor: "#fff", strokeWidth: 2,
            filter: ["==", ["get", "kind"], "run"],
          }));
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v3-selected", {
            color: "#a53a3a", radius: 12, strokeColor: "#fff", strokeWidth: 3,
            filter: ["==", ["get", "kind"], "selected"],
          }));
          map.layers.add(new atlas.layer.SymbolLayer(source, "planner-v3-labels", {
            textField: ["get", "label"], textOffset: [0, 1.25], textSize: 11,
            filter: ["!=", ["get", "kind"], "route"],
          }));
          setReady(true);
          setError(undefined);
      ...
      map.events.add("error", () => {
        if (!disposed) setError("Azure Maps reported an authentication or renderer error.");
      });
    } catch (exception) {
  ...
  }, [appClientId, mapsClientId, tenantId, ready]);
  ...
  if (!mapsClientId || !appClientId || !tenantId || error) {
    return <div style={{ display: "flex", flexDirection: "column", minHeight: 500, flex: 1 }}>
      <div style={{ padding: "10px 12px", borderBottom: "1px solid #e5edef", fontSize: ".78rem" }}>
        <strong>{error || "Azure Maps configuration is incomplete."}</strong> Using map fallback for the selected job.
      </div>
      <iframe
        title="Planner map fallback"
        src={`https://www.google.com/maps?q=${encodeURIComponent(fallbackQuery)}&output=embed`}
        style={{ border: 0, flex: 1, minHeight: 460, width: "100%" }}
        loading="lazy"
      />
    </div>;
  }

  return <div style={{ position: "relative", flex: 1, minHeight: 500 }}>
    <div ref={container} className="op-azure-map" style={{ height: "100%", minHeight: 500 }} />
    {!ready && <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", background: "rgba(247,250,251,.88)", zIndex: 2 }}>
      <div><strong>Loading planning map…</strong><br /><small>Azure Maps is initialising.</small></div>
    </div>}
    {ready && points.length === 0 && <div style={{ position: "absolute", top: 12, left: 12, right: 12, padding: 10, background: "rgba(255,255,255,.94)", border: "1px solid #dce7ea", borderRadius: 8, zIndex: 2 }}>
      <strong>Map connected, but no coordinates are available yet.</strong> Select a job with an address or map a collection site to coordinates.
    </div>}
    {ready && <div style={{ position: "absolute", bottom: 10, right: 10, padding: "5px 8px", background: "rgba(255,255,255,.9)", borderRadius: 6, fontSize: ".72rem", zIndex: 2 }}>
      {points.length} map point{points.length === 1 ? "" : "s"}
    </div>}
  </div>;
}

function PlannerV3Content() {
  ...
  const visibleJobs = useMemo(() => {
    ...
    const result = q ? jobs.filter((item) => [
  ...
  const sortedLoads = useMemo(() => [...loads].sort((a, b) =>
    PERIOD_ORDER[runPeriod(a)] - PERIOD_ORDER[runPeriod(b)] ||
    text(a.createdAtUtc).localeCompare(text(b.createdAtUtc)) ||
    text(a.reference).localeCompare(text(b.reference))), [loads]);

  const runNumberById = useMemo(() => new Map(sortedLoads.map((load, index) => [load.id, index + 1])), [sortedLoads]);
  ...
  useEffect(() => {
    ...
    const pending = [...addresses].filter((address) => !(address in geocodes)).slice(0, 80);
    ...
    void (async () => {
      ...
      for (let index = 0; index < pending.length; index += 6) {
        const entries = await Promise.all(pending.slice(index, index + 6).map(async (address) =>
          [address, (await geocode(address, accessToken)) || null] as const));
  ...
  async function stopsFor(item: PlannerJob, existing: LoadStop[] = []): Promise<PlannerStop[]> {
    ...
    const site = siteFor(sites, item.collectionSite);
    const collectionAddress = site?.collectionAddress || item.collectionSite;
    const hasCollection = existing.some((stop) =>
      normal(stop.name).includes(normal(item.collectionSite)) || normal(stop.address) === normal(collectionAddress));
    ...
    if (item.collectionSite !== "Collection not mapped" && !hasCollection) {
      ...
      if (collectionAddress && !(collectionAddress in geocodes)) {
        setGeocodes((current) => ({ ...current, [collectionAddress]: point || null }));
      }
      result.push({
        name: `Collect · ${item.collectionSite}`,
        address: site?.collectionAddress || undefined,
    ...
    result.push({
      ...
      address: item.deliveryAddress || item.destination || undefined,
      latitude: point?.latitude,
      longitude: point?.longitude,
    });
    return result;
  ...
  async function addToRun(load: Load, orderId: string) {
    ...
    try {
      ...
      setMessage(exception instanceof Error ? exception.message : "The job could not be added to the run.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function allocate(load: Load, vehicleId?: string, driverId?: string, trailerId?: string) {
    ...
    try {
      ...
      setMessage(exception instanceof Error ? exception.message : "Allocation could not be saved.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function suggestStopOrder(load: Load) {
    ...
    try {
      const accessToken = await token();
      const enriched = await Promise.all(currentStops.map(async (stop) => ({
        stop,
        point: stopCoordinate(stop) || (stop.address ? await geocode(stop.address, accessToken) : undefined),
      })));
      ...
      while (remaining.length) {
        ...
        remaining.forEach((candidate, index) => {
          const score = distanceScore(cursor, candidate.point);
      ...
      setMessage(`Run ${runNumberById.get(load.id) || ""} stop order updated.`);
    } catch (exception) {
      setMessage(exception instanceof Error ? exception.message : "Route suggestion could not be saved.");
    } finally {
      setBusy(false);
    }
  }
  ...
  const renderJob = (item: PlannerJob) => <article
    key={item.order.id}
    ...
    style={item.order.id === selectedJobId ? { border: "2px solid #a53a3a" } : undefined}
  >
    ...
    <div style={{ margin: "7px 0", display: "grid", gap: 4 }}>
      <div><small style={{ color: "#607685" }}>COLLECT</small><br /><b style={{ margin: 0 }}>{item.collectionSite}</b></div>
      <div style={{ color: "#78909c", fontWeight: 700 }}>↓</div>
      <div><small style={{ color: "#607685" }}>DELIVER</small><br /><b style={{ margin: 0 }}>{item.destination || item.depotId || item.order.reference}</b></div>
    </div>
    <small>{item.deliveryAddress || "Address / map point pending"}</small>
    <div className="op-job-meta">
      <span>{item.orderType}</span>
      <span>{item.orderType.toLowerCase().includes("crate") ? `${item.order.pallets ?? 0} crate units` : `${item.order.pallets ?? 0} plt`}</span>
      <span>{item.order.reference}</span>
  ...
  const renderRunGroup = (period: PlanningPeriod) => {
    const group = sortedLoads.filter((load) => runPeriod(load) === period);
    if (!group.length) return null;
    return <div style={{ display: "grid", gap: 8 }}>
      <div style={{ position: "sticky", top: 0, zIndex: 1, background: "#eef4f5", padding: "7px 8px", borderRadius: 7, fontWeight: 800, color: "#073a5a" }}>
        {period === "TBC" ? "TIME TBC" : `${period} RUNS`} · {group.length}
      </div>
      {group.map((load) => {
        const stops = safeStops(load);
        const selectedTrailer = trailers.find((item) => item.id === load.trailerId);
        const capacity = Number(load.totalPalletSpaces) || Number(selectedTrailer?.standardCapacity) || 26;
        const used = Number(load.palletSpacesUsed) || 0;
        const over = capacity > 0 && used > capacity;
        const selectedDriver = drivers.find((item) => item.id === load.driverId);
        const runNumber = runNumberById.get(load.id) || 0;
        return <article
          key={load.id}
        ...
        >
          <div className="op-run-heading">
            <div><strong>Run {runNumber}</strong><small>{period} · {text(load.status) || "Draft"}</small></div>
          ...
          <div style={{ margin: "7px 0" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: ".72rem", color: over ? "#a53a3a" : "#607685" }}>
              <span>Capacity</span><strong>{used} / {capacity}{over ? " · OVER" : ""}</strong>
            </div>
            <div style={{ height: 6, borderRadius: 6, background: "#e5edef", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${Math.min(100, capacity > 0 ? used / capacity * 100 : 0)}%`, background: over ? "#a53a3a" : "#007979" }} />
            </div>
          </div>
          <div className="op-allocation" onClick={(event) => event.stopPropagation()}>
            <select value={load.vehicleId || ""} disabled={busy} onChange={(event) => void allocate(load, event.target.value || undefined, load.driverId, load.trailerId)}>
              <option value="">Vehicle</option>
              {activeVehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id} disabled={!fleetioAvailable(vehicle)}>{vehicle.registration}{vehicle.fleetioStatus ? ` · ${vehicle.fleetioStatus}` : ""}</option>)}
            </select>
            <select value={load.driverId || ""} disabled={busy} onChange={(event) => void allocate(load, load.vehicleId, event.target.value || undefined, load.trailerId)}>
              <option value="">Driver</option>
              {activeDrivers.map((driver) => <option key={driver.id} value={driver.id}>{driver.displayName}{driver.tachoDriveAvailableTodayMinutes != null ? ` · ${hours(driver.tachoDriveAvailableTodayMinutes)}` : ""}</option>)}
            </select>
            <select value={load.trailerId || ""} disabled={busy} onChange={(event) => void allocate(load, load.vehicleId, load.driverId, event.target.value || undefined)}>
              <option value="">Trailer</option>
              {activeTrailers.map((trailer) => <option key={trailer.id} value={trailer.id}>{trailer.trailerNumber}{trailer.standardCapacity ? ` · ${trailer.standardCapacity} plt` : ""}</option>)}
            </select>
          </div>
          {selectedDriver && <small style={{ display: "block", marginTop: 5, color: "#607685" }}>{selectedDriver.displayName}: {hours(selectedDriver.tachoDriveAvailableTodayMinutes)}</small>}
          <button className="op-suggest" disabled={busy || stops.length < 2} onClick={(event) => { event.stopPropagation(); void suggestStopOrder(load); }}>Suggest stop order</button>
        </article>;
      })}
    </div>;
  };

  return <section className="operational-planner planner-v3">
    ...
    {suggestions.length > 0 && <section style={{ marginBottom: 12, border: "1px solid #dce7ea", borderRadius: 13, background: "#f7fafb" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 8, padding: 10 }}>
        {suggestions.slice(0, 4).map((item) => <article className="op-run-card" key={`${item.driverId}-${item.orderId}`}>
          <div className="op-run-heading"><div><strong>{item.driverName}</strong><small>Finished: {item.lastLocation}</small></div><span>{item.repositionMiles == null ? "distance ?" : `${item.repositionMiles} mi`}</span></div>
          <div className="op-run-route"><b>{item.collectionSite}</b> → {item.destination}</div>
          <div className="op-job-meta"><span>{item.orderType}</span><span>{item.orderReference}</span><span>{hours(item.driveAvailableTodayMinutes)}</span></div>
          <button className="primary" disabled={busy || !jobs.some((job) => job.order.id === item.orderId)} onClick={() => void createRun(item.orderId, item.driverId)}>Build suggested run</button>
        </article>)}
      </div>
    </section>}
    ...
    <div className="op-workspace">
      <section className="op-runs-column">
        <div className="op-column-heading"><div><p className="eyebrow">Runs</p><h2>Driver day builder</h2></div><span>{loads.length}</span></div>
      ...
      <section className="op-map-panel">
        <div className="op-column-heading">
          <div><p className="eyebrow">Map</p><h2>{selectedLoad ? `Run ${runNumberById.get(selectedLoad.id) || ""}` : selectedJob ? selectedJob.destination : "Collections & deliveries"}</h2></div>
          <span>{selectedLoad ? safeStops(selectedLoad).length : visibleJobs.length}</span>
        </div>
        <MapBoundary><PlannerMap jobs={visibleJobs} sites={sites} selectedLoad={selectedLoad} selectedJob={selectedJob} geocodes={geocodes} route={route} /></MapBoundary>
        <div className="op-map-legend"><span><i style={{ background: "#315f86" }} />Collection</span><span><i className="job-dot" />Delivery</span><span><i className="run-dot" />Selected run</span><span><i style={{ background: "#a53a3a" }} />Selected job</span></div>
      </section>
      ...
      <section className="op-jobs-column">
        ...
        <input className="op-job-search" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Search time, collection, delivery, customer, PO…" />
        {selectedJob && <aside className="op-selected-job"><strong>{selectedJob.period}{selectedJob.planningTime ? ` · ${selectedJob.planningTime}` : ""} · {selectedJob.collectionSite} → {selectedJob.destination}</strong><span>{selectedJob.deliveryAddress || "Delivery address / postcode not mapped"}</span></aside>}
        <div className="op-job-list">
          {amJobs.length > 0 && <><div style={{ fontWeight: 800, padding: "7px 8px", borderRadius: 7, background: "#eef4f5", color: "#073a5a" }}>AM JOBS · {amJobs.length}</div>{amJobs.map(renderJob)}</>}
          {pmJobs.length > 0 && <><div style={{ fontWeight: 800, padding: "7px 8px", borderRadius: 7, background: "#eef4f5", color: "#073a5a" }}>PM JOBS · {pmJobs.length}</div>{pmJobs.map(renderJob)}</>}
          {tbcJobs.length > 0 && <><div style={{ fontWeight: 800, padding: "7px 8px", borderRadius: 7, background: "#fff8e9", color: "#7b5100" }}>TIME TBC · {tbcJobs.length}</div>{tbcJobs.map(renderJob)}</>}
...
export function PlannerV3() {
  return <PlannerBoundary><PlannerV3Content /></PlannerBoundary>;
}
