...
type WorkbenchLoad = {
  ...
  stops?: Array<{ sequence: number; plannedArrivalUtc?: string }>;
};
...
function firstPlannedTime(load: WorkbenchLoad) {
  return [...(load.stops || [])].sort((a, b) => a.sequence - b.sequence).find(stop => stop.plannedArrivalUtc)?.plannedArrivalUtc || load.plannedStartUtc;
}
...
export function DriverDispatchOperational() {
  ...
  function captureDispatchClick(event: MouseEvent<HTMLDivElement>) {
    ...
    void openDispatchPreview(driverName);
  }

  async function sendDispatch() {
    if (!dialog?.load || !dialog.text.trim()) return;
    setSending(true);
    setDialog(current => current ? { ...current, error: undefined } : current);
    try {
      const access = await token();
      ...
      let readiness = await request<DispatchReadiness>(`/api/v1/loads/${encodeURIComponent(dialog.load.id)}/dispatch-readiness`, access, {
        method: "POST",
      ...
      if (!readiness.canDispatch && readiness.structuralReadiness?.classification === "Unverified" && readiness.structuralReadiness.requiresAcknowledgement) {
        ...
        acknowledged = true;
        readiness = await request<DispatchReadiness>(`/api/v1/loads/${encodeURIComponent(dialog.load.id)}/dispatch-readiness`, access, {
          method: "POST",
      ...
      await request(`/api/v1/loads/${encodeURIComponent(dialog.load.id)}/driver-message/sms`, access, {
        method: "POST",
        body: JSON.stringify({
          message: dialog.text,
          dispatch: true,
          routeDrivingMinutes: minutes,
      ...
      setDialog(current => current ? { ...current, error: error instanceof Error ? error.message : "Driver text could not be sent." } : current);
    } finally {
      setSending(false);
    }
  }
  ...
  return <div onClickCapture={captureDispatchClick}>
    ...
    {dialog && <div className="dispatch-modal-backdrop" role="dialog" aria-modal="true" aria-label="Dispatch text preview">
      <div className="dispatch-modal dispatch-text-first-modal">
        <div className="title-row">
          <div>
            <p className="eyebrow">Dispatch text preview</p>
            <h2>{dialog.load?.reference || dialog.driverName}</h2>
            <p className="hint">Review or edit the exact driver text. Route, Tacho and dispatch-readiness checks run when you press Send Dispatch.</p>
          </div>
          <button type="button" onClick={() => setDialog(undefined)} disabled={sending}>Close</button>
        </div>
        <textarea
          rows={14}
          value={dialog.text}
          disabled={dialog.loading || sending}
          onChange={event => setDialog(current => current ? { ...current, text: event.target.value } : current)}
        />
        {dialog.loading && <p className="hint">Loading the allocated run, driver, vehicle, trailer and stop details…</p>}
        {dialog.error && <p className="notice inline-notice" style={{ borderColor: "#b42318" }}>{dialog.error}</p>}
        <div className="dispatch-modal-actions">
          <button type="button" onClick={() => setDialog(undefined)} disabled={sending}>Cancel</button>
          <button className="primary" type="button" onClick={() => void sendDispatch()} disabled={dialog.loading || sending || !dialog.load || !dialog.text.trim()}>{sending ? "Sending…" : "SEND DISPATCH"}</button>
        </div>
      </div>
    </div>}
  </div>;
