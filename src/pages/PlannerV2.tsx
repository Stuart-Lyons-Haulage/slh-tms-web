...
function inferOrderType(order: TransportOrder) {
  const explicit = tagged(order.driverInstructions, "Order type");
  if (explicit) return explicit;
  const combined = `${order.driverInstructions || ""} ${order.stallNumber || ""} ${order.marketName || ""}`;
  return /\bcrate(s)?\b/i.test(combined) ? "Crates" : "Pallets";
}
...
function siteFor(sites: Site[], name: string) {
  ...
  return sites.find((site) =>
    [site.externalCode, site.name, site.driverTextName, ...(site.aliases || "").split(/[,;|]/)]
...
function safeStops(load?: Load): LoadStop[] {
  return safeArray<LoadStop>(load?.stops).filter((stop) => Boolean(stop && typeof stop === "object"));
}
...
function fleetioAvailable(vehicle: Vehicle) {
  ...
  return !["out of service", "out-of-service", "inactive", "sold", "vor"].some((phrase) => status.includes(phrase));
}

async function geocode(address: string, accessToken: string): Promise<Coordinate | undefined> {
...
class MapBoundary extends Component<{ children: ReactNode }, { error?: Error }> {
  ...
  render() {
    ...
    return <div className="op-map-empty">
      <div style={{ maxWidth: 520, padding: 24 }}>
        <strong>Map renderer unavailable</strong>
        <p>The run builder remains usable. Map failure has been isolated from the Planner.</p>
...
function PlannerMap({
  ...
  const sourceRef = useRef<atlas.source.DataSource>();
  ...
  const points = useMemo<MapPoint[]>(() => {
    ...
    for (const item of jobs) {
      ...
      const collectionAddress = collectionSite?.collectionAddress || item.collectionSite;
      const collection = siteCoordinate(collectionSite) || geocodes[collectionAddress];
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.events.add("ready", () => {
        ...
        try {
          ...
          map.layers.add(new atlas.layer.LineLayer(source, "planner-v2-route", {
            strokeColor: "#007979",
            strokeWidth: 5,
            filter: ["==", ["get", "kind"], "route"],
          }));
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v2-delivery", {
            color: "#e3912e",
            radius: 8,
            strokeColor: "#fff",
            strokeWidth: 2,
          ...
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v2-collection", {
            color: "#315f86",
            radius: 9,
            strokeColor: "#fff",
            strokeWidth: 2,
          ...
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v2-run", {
            color: "#007979",
            radius: 10,
            strokeColor: "#fff",
            strokeWidth: 2,
          ...
          map.layers.add(new atlas.layer.BubbleLayer(source, "planner-v2-selected", {
            color: "#a53a3a",
            radius: 12,
            strokeColor: "#fff",
            strokeWidth: 3,
          ...
          map.layers.add(new atlas.layer.SymbolLayer(source, "planner-v2-labels", {
            textField: ["get", "label"],
            textOffset: [0, 1.25],
            textSize: 11,
            filter: ["!=", ["get", "kind"], "route"],
      ...
      map.events.add("error", () => {
        if (!disposed) setError("Azure Maps reported a renderer or authentication error.");
      });
    } catch (exception) {
    ...
    return () => {
      ...
      try { mapRef.current?.dispose(); } catch { /* map cleanup is best effort */ }
      mapRef.current = undefined;
    };
  }, [appClientId, mapsClientId, tenantId]);

  useEffect(() => {
    if (!ready || !sourceRef.current || !mapRef.current) return;
    try {
      ...
      if (route.length > 1) {
        source.add(new atlas.data.Feature(new atlas.data.LineString(route), { kind: "route" }));
      }
      const camera = [
      ...
      if (camera.length > 1) {
        mapRef.current.setCamera({ bounds: atlas.data.BoundingBox.fromPositions(camera), padding: 70 });
      } else if (camera.length === 1) {
        mapRef.current.setCamera({ center: camera[0], zoom: 11 });
      }
    } catch (exception) {
  ...
  if (!mapsClientId || !appClientId || !tenantId) {
    return <div className="op-map-empty">
      <div style={{ maxWidth: 520, padding: 24 }}>
        <strong>Map configuration incomplete</strong>
        <p>The Planner remains usable, but Azure Maps credentials are not available to this web build.</p>
      </div>
    </div>;
  }
  ...
  if (error) {
    return <div className="op-map-empty">
      <div style={{ maxWidth: 520, padding: 24 }}>
        ...
        <p>{error}</p>
        <small>The run builder and job allocation controls remain available.</small>
      </div>
    </div>;
  }
...
export function PlannerV2() {
  ...
  const visibleJobs = useMemo(() => {
    ...
    return [...source].sort((a, b) =>
      a.collectionSite.localeCompare(b.collectionSite) || a.destination.localeCompare(b.destination),
    );
  }, [filter, jobs]);
  const suggestions = useMemo(
    () => safeArray<PlannerDaySuggestion>(suggestionsApi.data?.suggestions),
    [suggestionsApi.data],
  );
  ...
  }, [driversApi, loadsApi, ordersApi, sitesApi, suggestionsApi, trailersApi, vehiclesApi]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for (let index = 0; index < pending.length; index += 8) {
        ...
        const entries = await Promise.all(batch.map(async (address) =>
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const result = await api.route(selectedLoadId, await token()) as {
          routes?: Array<{ legs?: Array<{ points?: Array<{ latitude?: number; longitude?: number }> }> }>;
        };
        const line = safeArray(result.routes?.[0]?.legs).flatMap((leg) =>
          safeArray(leg.points).flatMap((point) =>
  ...
  async function stopsFor(item: PlannerJob, existing: LoadStop[] = []): Promise<PlannerStop[]> {
    ...
    const hasCollection = existing.some((stop) =>
      normal(stop.name).includes(normal(item.collectionSite)) ||
      normal(stop.address) === normal(collectionAddress),
    );
    if (item.collectionSite && item.collectionSite !== "Collection not mapped" && !hasCollection) {
    ...
    if (deliveryAddress) {
      setGeocodes((current) => ({ ...current, [deliveryAddress]: point || null }));
    }
    result.push({
  ...
  async function createRun(orderId: string, suggestedDriverId?: string) {
    ...
    try {
      ...
      setMessage(`${item.collectionSite} → ${item.destination} added to new run ${created.reference}.`);
    } catch (exception) {
  ...
  async function addToRun(load: Load, orderId: string) {
    ...
    try {
      ...
      const used = Number(load.palletSpacesUsed) || safeStops(load)
        .filter((stop) => stop.orderId)
        .reduce((sum) => sum, 0);
      await api.updateLoadUtilisation(load.id, {
        palletSpacesUsed: used + capacityUnits(item),
  ...
  async function allocate(
    ...
    try {
      ...
      if (trailerId) {
        ...
        const capacity = Number(trailer?.standardCapacity) || Number(load.totalPalletSpaces) || 26;
        await api.updateLoadUtilisation(load.id, {
          palletSpacesUsed: Number(load.palletSpacesUsed) || 0,
          totalPalletSpaces: capacity,
  ...
  async function suggestStopOrder(load: Load) {
    ...
    try {
      ...
      const enriched = await Promise.all(currentStops.map(async (stop) => {
        const point = stopCoordinate(stop) || (stop.address ? await geocode(stop.address, accessToken) : undefined);
        return { stop, point };
      }));
      const collections = enriched.filter((item) => text(item.stop.name).toLowerCase().startsWith("collect"));
      const remaining = enriched.filter((item) => !text(item.stop.name).toLowerCase().startsWith("collect"));
      const ordered = [...collections];
      let cursor = collections.map((item) => item.point).filter(Boolean).at(-1);
      ...
      setSelectedLoadId(load.id);
      setMessage(`${load.reference} stop order updated using the available map points.`);
    } catch (exception) {
  ...
  function readDroppedOrder(event: DragEvent) {
    return event.dataTransfer.getData("text/slh-order-id") || event.dataTransfer.getData("text/plain");
  }
  ...
  const mapError = ordersApi.error || loadsApi.error || sitesApi.error;

  return <section className="operational-planner planner-v2">
    <div className="op-planner-header">
      <div>
        <p className="eyebrow">Planner V2</p>
        <h1>Build the day visually</h1>
        <p>See collection → delivery, build runs by click or drag, allocate resources, then refine the route on the map.</p>
      </div>
      <div className="op-header-actions">
        ...
        <button disabled={busy} onClick={() => void refreshAll()}>{busy ? "Working…" : "Refresh all"}</button>
      </div>
    </div>
    ...
    <div className="op-metrics">
      ...
      <span><strong>{jobs.filter((item) => item.orderType.toLowerCase().includes("crate")).length}</strong> crate jobs</span>
      <span><strong>{suggestions.length}</strong> positioning suggestions</span>
    </div>

    {message && <p className="notice inline-notice">{message}</p>}
    {mapError && <p className="notice inline-notice">{mapError}</p>}

    {suggestions.length > 0 && <section className="planner-v2-suggestions" style={{ marginBottom: 12 }}>
      <div className="op-column-heading">
        <div><p className="eyebrow">SLH Assistant</p><h2>Previous-day positioning suggestions</h2></div>
        <span>{suggestions.length}</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 8, padding: 10 }}>
        {suggestions.slice(0, 6).map((item) => <article className="op-run-card" key={`${item.driverId}-${item.orderId}`}>
          <div className="op-run-heading">
            <div><strong>{item.driverName}</strong><small>Finished: {item.lastLocation}</small></div>
            <span>{item.repositionMiles == null ? "distance ?" : `${item.repositionMiles} mi`}</span>
          </div>
          ...
          <div className="op-job-meta">
            ...
            <span>{hours(item.driveAvailableTodayMinutes)}</span>
          </div>
          <p style={{ fontSize: ".75rem", color: "#607685", margin: "7px 0" }}>{item.reason}</p>
          <div style={{ display: "flex", gap: 6 }}>
            <button disabled={busy} onClick={() => {
              setFilter(item.orderReference);
              setSelectedJobId(item.orderId);
            }}>Find job</button>
    ...
    <div className="op-workspace">
      <section className="op-runs-column">
        ...
        <div className="op-new-run" onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
          event.preventDefault();
          const id = readDroppedOrder(event);
          if (id) void createRun(id);
        ...
        <div className="op-run-list">
          {loads.map((load) => {
            ...
            >
              <div className="op-run-heading">
                <div><strong>{text(load.reference) || "Unnamed run"}</strong><small>{text(load.status) || "Draft"}</small></div>
              ...
              <div style={{ margin: "7px 0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: ".72rem", color: over ? "#a53a3a" : "#607685" }}>
                  <span>Capacity</span><strong>{used} / {capacity} pallet spaces{over ? " · OVER" : ""}</strong>
                </div>
              ...
              <div className="op-allocation" onClick={(event) => event.stopPropagation()}>
                <select value={load.vehicleId || ""} disabled={busy} onChange={(event) => {
                  const value = event.target.value || undefined;
                  void allocate(load, value, load.driverId, load.trailerId);
                }}>
                  <option value="">Vehicle</option>
                  {activeVehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id} disabled={!fleetioAvailable(vehicle)}>
                    {vehicle.registration}{vehicle.fleetioStatus ? ` · ${vehicle.fleetioStatus}` : ""}
                  </option>)}
                </select>
                <select value={load.driverId || ""} disabled={busy} onChange={(event) => {
                  const value = event.target.value || undefined;
                  void allocate(load, load.vehicleId, value, load.trailerId);
                }}>
                  <option value="">Driver</option>
                  {activeDrivers.map((driver) => <option key={driver.id} value={driver.id}>
                    {driver.displayName}{driver.tachoDriveAvailableTodayMinutes != null ? ` · ${hours(driver.tachoDriveAvailableTodayMinutes)}` : ""}
                  </option>)}
                </select>
                <select value={load.trailerId || ""} disabled={busy} onChange={(event) => {
                  const value = event.target.value || undefined;
                  void allocate(load, load.vehicleId, load.driverId, value);
                }}>
                  <option value="">Trailer</option>
                  {activeTrailers.map((trailer) => <option key={trailer.id} value={trailer.id}>
                    {trailer.trailerNumber}{trailer.standardCapacity ? ` · ${trailer.standardCapacity} plt` : ""}
                  </option>)}
                </select>
              </div>
              {selectedDriver && <small style={{ display: "block", marginTop: 5, color: "#607685" }}>
                {selectedDriver.displayName}: {hours(selectedDriver.tachoDriveAvailableTodayMinutes)}
              </small>}
              <button className="op-suggest" disabled={busy || stops.length < 2} onClick={(event) => {
                event.stopPropagation();
                void suggestStopOrder(load);
              }}>Suggest stop order</button>
            </article>;
          })}
          {!loads.length && !loadsApi.loading && <p className="op-empty">No runs yet. Create one from a Ready to Plan job.</p>}
        </div>
      </section>

      <section className="op-map-panel">
        <div className="op-column-heading">
          <div><p className="eyebrow">Map</p><h2>{selectedLoad ? selectedLoad.reference : "Collections & deliveries"}</h2></div>
        ...
        <div className="op-map-legend">
          <span><i style={{ background: "#315f86" }} />Collection</span>
          <span><i className="job-dot" />Delivery</span>
          <span><i className="run-dot" />Selected run</span>
          <span><i style={{ background: "#a53a3a" }} />Selected job</span>
        </div>
      </section>
      ...
      <section className="op-jobs-column">
        ...
        <input className="op-job-search" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Search collection, delivery, customer, PO…" />
        {selectedJob && <aside className="op-selected-job">
          <strong>{selectedJob.collectionSite} → {selectedJob.destination}</strong>
          <span>{selectedJob.deliveryAddress || "Delivery address / postcode not mapped"}</span>
          <span>{selectedJob.orderType} · {selectedJob.order.pallets ?? 0} {selectedJob.orderType.toLowerCase().includes("crate") ? "units" : "pallets"}</span>
        </aside>}
        <div className="op-job-list">
          {visibleJobs.map((item) => {
            const isSelected = item.order.id === selectedJobId;
            return <article
              key={item.order.id}
              ...
              style={isSelected ? { border: "2px solid #a53a3a" } : undefined}
            >
              <div className="op-job-heading">
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 8 }} onClick={(event) => event.stopPropagation()}>
                ...
                <button disabled={busy || !selectedLoad} onClick={() => selectedLoad && void addToRun(selectedLoad, item.order.id)}>
                  {selectedLoad ? `Add to ${selectedLoad.reference}` : "Select run first"}
                </button>
              </div>
            </article>;
          })}
