    ...
    <section>
      ...
      >
        ...
        <div className="panel">
          <h2>Planner focus</h2>
          <p>
            Plan today’s work, allocate the fleet, set route points and
            calculate ETAs from the Planner.
          </p>
          <Link to="/">Open planner →</Link>
        </div>
      </State>
    </section>
  );
  ...
  : selected.driverSource === "DOT/Falcon"
    ...
    : "Planned driver"}
              ...
              {selected.loadId && (
                ...
                >
                  Open job {selected.loadReference || ""} →
                </a>
              )}
              {nextEta && (
                <aside
            ...
            {selected.tacho ? (
              <div className="tacho-detail">
                ...
                <small>
                  Long days this week{" "}
                  {selected.tacho.longDaysWorkedThisWeek ?? "—"} · shortened
                  daily rests{" "}
                  {selected.tacho.shortDailyRestTakenThisWeek ?? "—"}
                </small>
    ...
    : "Planned"}
                ...
                {vehicle.loadId ? (
                  ...
                  >
                    ...
                    <span>{vehicle.loadStatus || "Planned"} →</span>
                  </a>
                ) : (
      ...
      <div className="field-grid">
        ...
        {orderKind === "market" ? (
          ...
          <>
            ...
            <label>
              ...
              >
                <option value="">Optional site/map…</option>
                {(sites.data || [])
        ...
        <label className="wide">
          ...
          <textarea
            ...
            placeholder="Collection point, access notes, goods handling…"
          />
        </label>
      </div>
  ...
  return (
    <section>
      ...
      <div className="planner-toolbar">
        ...
        >
          {saving ? "Saving…" : `Build load (${selected.length})`}
        </button>
      </div>
      ...
      <div className="planner-workspace">
        <article className="lane unallocated">
          ...
          {ordersApi.loading && !ordersApi.data ? (
            <p className="lane-state">Loading approved orders…</p>
          ) : ordersApi.error ? (
        ...
        <article className="lane planner-lanes">
          ...
          {loads.loading && !loads.data ? (
            <p className="lane-state">Loading planned loads…</p>
          ) : loads.error ? (
            <div className="lane-state error">
              <p>{loads.error}</p>
  ...
  return (
    <section className="resource-board">
      <div className="resource-board-heading">
        ...
        <span>{saving ? "Saving allocation…" : "Changes save immediately"}</span>
      </div>
      <div className="drag-loads">
        {loads.map((load) => (
          ...
          >
            ...
            <small>
              {load.stops.length} stops · {load.driverId ? "driver ✓" : "driver needed"} ·{" "}
              {load.vehicleId ? "vehicle ✓" : "vehicle needed"}
            </small>
          </article>
        ))}
      </div>
  ...
  if (loading)
    return (
      <aside className="return-suggestions">
        <p>Checking drivers already away…</p>
      </aside>
    );
  if (error)
    return (
    ...
    <div className="commercial-editor">
      ...
      {open && (
        <div className="commercial-fields">
          ...
          >
            {saving ? "Saving…" : "Save commercial control"}
          </button>
          {message && <p className="hint">{message}</p>}
        </div>
      )}
      ...
      >
        ...
        <section className="forecast-panel">
          ...
          {forecast.loading && !forecast.data ? (
            <p className="lane-state">Calculating the seven-day picture…</p>
          ) : forecast.error ? (
            <div className="lane-state error">
              <p>{forecast.error}</p>
...
function MasterValidation({
  ...
  return (
    ...
    >
      ...
      {suggestions.loading ? (
        <span>Checking linked master-data rules…</span>
      ) : combined.length ? (
        <ul>
...
function DriverQuickAdd({ onSaved }: { onSaved: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button className="primary" disabled={saving}>
        {saving ? "Saving…" : "Send driver for review"}
      </button>
      {message && <p className="hint">{message}</p>}
    </form>
  );
  ...
  if (loading)
    return (
      <div className="panel fleetio-panel">
        <strong>Checking Fleetio vehicle alignment…</strong>
      </div>
    );
  if (error)
    return (
            ...
            <tbody>
              {data.records.slice(0, 100).map((row, index) => (
                ...
                >
                  ...
                  <td>
                    {row.tmsRegistration || row.fleetioRegistration || "—"}
                  </td>
                  <td>{row.fleetioStatus || "—"}</td>
                  <td>
                    {row.fleetioVor ? (
...
export function FuelMaster() {
  ...
  const issues = rows.length
    ? []
    : ["No fuel prices loaded yet. Add this week’s provider prices below."];
  return (
    <section>
      <div className="title-row">
        <div>
      ...
      <div className="metrics">
        ...
        <Metric
          label="Pricing max"
          value={String(
            rows.find((row) => row.isPricingMaximum)?.pricePencePerLitre ?? "—",
          )}
    ...
    <section>
      <div className="title-row">
        ...
        <button onClick={() => void linkRegistered()} disabled={linking}>
          {linking ? "Linking…" : "Link registered rows"}
        </button>
      </div>
    ...
    <div className="master-counts">
      {cards.map(([label, value]) => (
        <article key={label} className={value?.ok === false ? "error" : ""}>
          <span>{label}</span>
          <strong>{value?.ok === false ? "!" : (value?.count ?? "—")}</strong>
  ...
  async function submit() {
    ...
    try {
      ...
      for (let start = 0; start < records.length; start += batchSize) {
        ...
        setSummary(
          `Applying ${start + 1}-${start + batch.length} of ${records.length} records…`,
        );
      ...
      <div className="field-grid">
        ...
        <label className="wide">
          ...
          <textarea
            ...
            placeholder="Access, gate, stand or collection notes…"
          />
        </label>
      </div>
      <button className="primary" disabled={saving}>
        {saving ? "Saving…" : "Send site for review"}
      </button>
      {message && <p className="hint">{message}</p>}
    </form>
  );
          ...
          <article className="admin-card" key={setting.label}>
            ...
            {setting.label === "Sage HR" && (
              <>
                ...
                >
                  {syncing ? "Syncing…" : "Sync drivers from Sage"}
                </button>
              </>
            )}
            {setting.label === "Fleetio" && (
              <>
                ...
                >
                  {fleetioSyncing ? "Syncing…" : "Sync vehicles from Fleetio"}
                </button>
              </>
            )}
          </article>
