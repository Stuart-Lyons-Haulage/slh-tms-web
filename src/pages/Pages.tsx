...
function ReconnectMicrosoft() { const { instance } = useMsal(); return <button className="primary" onClick={() => void instance.loginRedirect({ scopes: apiScope ? [apiScope] : [], prompt: 'select_account' })}>Reconnect Microsoft</button>; }
function State({ loading, error, empty, children }: { loading: boolean; error?: string; empty?: boolean; children: ReactNode }) { if (loading) return <div className="state">Loading operational data…</div>; if (error) return <div className="state error"><p>{error}</p>{error.includes('Microsoft sign-in') && <ReconnectMicrosoft />}</div>; if (empty) return <div className="state">No records are available for this view.</div>; return <>{children}</>; }
...
const formatMinutes = (value?: number) => value == null ? 'Not returned' : `${Math.floor(value / 60)}h ${String(value % 60).padStart(2, '0')}m`;
...
function FleetRollout({ fleet }: { fleet?: FleetStatus }) {
  ...
  const vehicles = [...(fleet?.vehicles || [])].sort((left, right) => dueTime(left.plannedDutyUtc) - dueTime(right.plannedDutyUtc) || left.registration.localeCompare(right.registration));
  ...
  return <div className="rollout-panel"><div className="rollout-heading"><div><p className="eyebrow">Live vehicle status</p><h2>Vehicle, card-holder and current job</h2></div><span>{fleet?.attentionCount || 0} to watch</span></div>{selected && <div className="vehicle-detail-panel"><div className="vehicle-detail-main"><div><strong>{selected.registration}</strong><span>{fleetConditionLabel(selected.condition)} · {selected.trackingIdentifier || selected.fleetNumber || 'RoadTech match pending'}</span>{selected.driverName && <small className="live-driver-name">{selected.driverSource === 'TachoMaster' ? 'TachoMaster driver' : 'Planned driver'}: {selected.driverName}</small>}{selected.tacho?.cardNumber && <small>Driver card: {selected.tacho.cardNumber}</small>}{selected.driverMismatch && <small className="driver-mismatch">TachoMaster driver differs from allocated driver {selected.allocatedDriverName}.</small>}<small>{selected.lastEventTimeUtc ? 'Vehicle update ' + formatDate(selected.lastEventTimeUtc) + ' · ' + (selected.speedKph ?? 0) + ' km/h' : 'No live DOT point matched yet'}</small>{selected.loadId && <a className="job-link" href={`/?loadId=${encodeURIComponent(selected.loadId)}`}>Open job {selected.loadReference || ''} →</a>}{selected.fleetioStatus && <small className={'fleetio-chip ' + fleetioStatusClass(selected.fleetioStatus)}>Fleetio: {selected.fleetioStatus}{selected.fleetioName ? ' · ' + selected.fleetioName : ''}</small>}</div>{selected.tacho ? <div className="tacho-detail"><div className="tacho-metrics"><article><span>Driving used</span><strong>{formatMinutes(selected.tacho.driveMinutes)}</strong></article><article><span>Driving left today</span><strong>{formatMinutes(selected.tacho.driveAvailableTodayMinutes)}</strong></article><article><span>Rest / break recorded</span><strong>{formatMinutes(selected.tacho.restMinutes)}</strong><small>{selected.tacho.breakCount ? `${selected.tacho.breakCount} WTD break${selected.tacho.breakCount === 1 ? '' : 's'} · ${formatMinutes(selected.tacho.breakMinutes)}` : 'No separate WTD break returned'}</small></article><article><span>Work left this week</span><strong>{formatMinutes(selected.tacho.workAvailableWeekMinutes)}</strong></article></div><small>Duty from {formatDate(selected.tacho.dutyStartUtc)}{selected.tacho.dutyEndUtc ? ` to ${formatDate(selected.tacho.dutyEndUtc)}` : ' · currently open in TachoMaster'}</small><small>Driving available: week {formatMinutes(selected.tacho.driveAvailableWeekMinutes)} · fortnight {formatMinutes(selected.tacho.driveAvailableFortnightMinutes)}</small><small>Long days this week {selected.tacho.longDaysWorkedThisWeek ?? '—'} · shortened daily rests {selected.tacho.shortDailyRestTakenThisWeek ?? '—'}</small><small className="tacho-valid">Tacho figures valid at {formatDate(selected.tacho.metricsValidAtUtc)}</small></div> : <p className="tacho-unavailable">No TachoMaster duty or driver-card record has been returned for this vehicle.</p>}</div>{selectedMapUrl ? <a className="vehicle-map-link" href={selectedMapUrl} target="_blank" rel="noreferrer">Open map location →</a> : <span className="hint">No latitude/longitude returned for this vehicle yet.</span>}</div>}{vehicles.length ? <div className="rollout-grid">{vehicles.map(vehicle => {
    const hasMap = vehicle.latitude != null && vehicle.longitude != null;
    return <article className={`rollout-card ${vehicleStatusClass(vehicle)} ${selectedVehicleId === vehicle.vehicleId ? 'selected' : ''}`} key={vehicle.vehicleId}><button type="button" className="rollout-select" onClick={() => setSelectedVehicleId(vehicle.vehicleId)}><span className="condition-dot" /><span className="rollout-vehicle"><strong>{vehicle.registration}</strong><small>{vehicle.fleetNumber || vehicle.trackingIdentifier || 'Fleet vehicle'}</small></span><span className="rollout-state"><b>{fleetConditionLabel(vehicle.condition)}</b>{vehicle.driverName && <small className="rollout-driver">{vehicle.driverSource === 'TachoMaster' ? 'Tacho' : 'Planned'}: {vehicle.driverName}</small>}{vehicle.driverMismatch && <small className="driver-mismatch">Allocated: {vehicle.allocatedDriverName}</small>}<small>{vehicle.plannedDutyUtc ? `Due ${formatDate(vehicle.plannedDutyUtc)}` : hasMap ? 'Live RoadTech point' : 'No planned sign-on time'}</small><small>{vehicle.lastEventTimeUtc ? `${vehicle.speedKph ?? 0} km/h · ${formatDate(vehicle.lastEventTimeUtc)}` : 'Awaiting RoadTech match today'}</small>{vehicle.fleetioStatus && <small className={`fleetio-chip ${fleetioStatusClass(vehicle.fleetioStatus)}`}>Fleetio: {vehicle.fleetioStatus}</small>}</span></button>{vehicle.loadId ? <a className="rollout-job" href={`/?loadId=${encodeURIComponent(vehicle.loadId)}`}><strong>{vehicle.loadReference || 'Open current job'}</strong><span>{vehicle.loadStatus || 'Planned'} →</span></a> : <span className="rollout-job empty">No current job</span>}</article>;
...
function fleetConditionLabel(condition: FleetStatus['vehicles'][number]['condition']) { return ({ Moving: 'Moving', Started: 'Engine on · stationary', Parked: 'Not active', Stationary: 'Not active', SignedOn: 'TachoMaster duty · stationary', Stale: 'Tracking stale', NotSignedOn: 'Not active' })[condition]; }
function vehicleStatusClass(vehicle: FleetStatus['vehicles'][number]) { return vehicle.condition === 'Moving' ? 'signedon' : vehicle.condition === 'Stale' ? 'stale' : 'parked'; }
...
function cleanOrderRow(row: ImportRow, index: number, source: string): ImportRow | undefined {
  ...
  const collectionDate = orderDate(row.collectionDate) || orderDate(row.deliveryDate) || localDate();
...
function QuickOrderForm() {
  ...
  const [form, setForm] = useState({ poNumber: '', customerCode: '', collectionDate: localDate(), deliveryDate: '', deliveryWindowStartUtc: '', deliveryWindowEndUtc: '', pallets: '', averagePalletWeightKg: '', estimatedWeightKg: '', sellerName: '', marketName: '', stallNumber: '', senderName: '', driverInstructions: '', mapLink: '' });
  const [message, setMessage] = useState<string>();
...
export function PlanningBoard() {
  ...
  const returns = useApi(useCallback(async () => api.returnLoadSuggestions(date, await token()), [date, token]));
  const [selected, setSelected] = useState<string[]>([]);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<string>();
  const today = planningOrders(ordersApi.data);
  const planned = loads.data || [];
  const incompleteLoads = planned.filter(load => !load.driverId || !load.vehicleId).length;
  const selectedOrders = today.filter(order => selected.includes(order.id));
  const toggle = (id: string) => setSelected(current => current.includes(id) ? current.filter(value => value !== id) : [...current, id]);
  const refreshAll = () => { void ordersApi.refresh(); void loads.refresh(); void returns.refresh(); };
  async function buildLoad() {
    const chosen = today.filter(order => selected.includes(order.id)); if (!chosen.length) return;
    setSaving(true); setMessage(undefined);
    try {
      await api.createLoad({ reference: `LOAD-${date.replaceAll('-', '')}-${String(planned.length + 1).padStart(2, '0')}`, planningDate: date, stops: chosen.map(order => ({ orderId: order.id, name: `${order.poNumber} · ${order.customerCode}` })) }, await token());
      setSelected([]); await loads.refresh(); await returns.refresh();
      setMessage('Load saved. Allocate the fleet, add addresses and calculate route/ETA. Market details are retained for dispatch.');
    } catch (exception) { setMessage(exception instanceof Error ? exception.message : 'Could not save the load.'); }
    finally { setSaving(false); }
  }
  return <section><div className="title-row"><div><p className="eyebrow">Plan and control</p><h1>Planning board</h1><p className="intro">Build the day from approved work, then allocate and route each load without losing the board if one service is slow.</p></div><button onClick={refreshAll}>Refresh board</button></div><div className="planner-health"><article><span>Ready to plan</span><strong>{today.length}</strong><small>Approved orders</small></article><article><span>Planned loads</span><strong>{planned.length}</strong><small>Saved for this day</small></article><article className={incompleteLoads ? 'attention' : ''}><span>Need allocation</span><strong>{incompleteLoads}</strong><small>Driver or vehicle missing</small></article></div><div className="planner-toolbar"><label>Plan date <input type="date" value={date} onChange={event => { setDate(event.target.value); setSelected([]); }} /></label><span>{today.length} approved order{today.length === 1 ? '' : 's'} awaiting load allocation</span><button className="primary" disabled={!selected.length || saving} onClick={() => void buildLoad()}>{saving ? 'Saving…' : `Build load (${selected.length})`}</button></div><DriverReturnSuggestions data={returns.data} loading={returns.loading} error={returns.error} /><PlanningSuggestions orders={today} selected={selected} onSelect={setSelected} /><SelectedLoadPreview orders={selectedOrders} />{message && <p className="notice inline-notice">{message}</p>}<div className="planner-workspace"><article className="lane unallocated"><h2>Ready to plan <span>{today.length}</span></h2>{ordersApi.loading && !ordersApi.data ? <p className="lane-state">Loading approved orders…</p> : ordersApi.error ? <div className="lane-state error"><p>{ordersApi.error}</p>{ordersApi.error.includes('Microsoft sign-in') && <ReconnectMicrosoft />}<button onClick={() => void ordersApi.refresh()}>Retry orders</button></div> : today.length ? today.map(order => <OrderCard key={order.id} order={order} selected={selected.includes(order.id)} onToggle={() => toggle(order.id)} />) : <p className="hint">No approved orders are ready for this date. Review new orders in Staging.</p>}</article><article className="lane planner-lanes"><h2>Planned loads <span>{planned.length}</span></h2>{loads.loading && !loads.data ? <p className="lane-state">Loading planned loads…</p> : loads.error ? <div className="lane-state error"><p>{loads.error}</p>{loads.error.includes('Microsoft sign-in') && <ReconnectMicrosoft />}<button onClick={() => void loads.refresh()}>Retry loads</button></div> : planned.length ? planned.map(load => <LoadCard key={load.id} load={load} onSaved={loads.refresh} />) : <p className="hint">Create a load from approved orders to allocate it here.</p>}</article><OperationalMap loads={planned} telemetry={undefined} /></div></section>;
}
...
function telemetryVehicle(vehicleId: string, registration: string, fleetNumber: string | undefined, record: Telemetry['records'][number]): FleetStatus['vehicles'][number] {
  ...
  const ageMinutes = Math.max(0, Math.floor((Date.now() - eventTime.getTime()) / 60_000));
  const today = localDate();
  const condition: FleetStatus['vehicles'][number]['condition'] = record.eventTimeUtc.slice(0, 10) !== today ? 'NotSignedOn' : ageMinutes > 30 ? 'Stale' : record.isMoving || (record.speedKph || 0) > 3 ? 'Moving' : 'NotSignedOn';
  return { vehicleId, registration, fleetNumber, trackingIdentifier: record.vehicleIdentifier, condition, lastEventTimeUtc: record.eventTimeUtc, ageMinutes, isMoving: record.isMoving, speedKph: record.speedKph, latitude: record.latitude, longitude: record.longitude };
}
...
export function LiveTracking() { const token = useAccessToken(); const [date, setDate] = useState(localDate()); const [mode, setMode] = useState<'live' | 'history'>('live'); const fleet = useApi(useCallback(async () => loadFleetStatus(await token()), [token])); const roadTech = useApi(useCallback(async () => api.roadTechStatus(await token()), [token])); const tachoMaster = useApi(useCallback(async () => api.tachoMasterStatus(await token()), [token])); const history = useApi(useCallback(async () => api.trackingHistory(date, await token()), [date, token])); const loading = mode === 'live' ? fleet.loading || roadTech.loading : history.loading; const error = mode === 'live' ? fleet.error : history.error; const refresh = mode === 'live' ? async () => { await Promise.all([fleet.refresh(), roadTech.refresh(), tachoMaster.refresh()]); } : history.refresh; return <section><div className="title-row"><div><p className="eyebrow">RoadTech Falcon + TachoMaster</p><h1>Live fleet control</h1></div><button onClick={() => void refresh()}>Refresh</button></div><div className="tracking-controls"><button className={mode === 'live' ? 'active' : ''} onClick={() => setMode('live')}>Live fleet</button><button className={mode === 'history' ? 'active' : ''} onClick={() => setMode('history')}>Historic day</button>{mode === 'history' && <label>Tracking date <input type="date" value={date} onChange={event => setDate(event.target.value)} /></label>}</div><State loading={loading} error={error}>{mode === 'live' ? <><div className="panel"><strong>{fleet.data?.vehicleCount || 0} fleet vehicles</strong><span>{fleet.data?.readyCount || 0} moving now · {fleet.data?.attentionCount || 0} stale or not signed on · refreshed {formatDate(fleet.data?.retrievedAtUtc)}</span></div>{roadTech.data && <p className={roadTech.data.connected ? 'notice ready' : 'notice'}>{roadTech.data.message}</p>}{tachoMaster.data && <p className={tachoMaster.data.connected ? 'notice ready' : 'notice'}>{tachoMaster.data.message}</p>}{tachoMaster.error && <p className="notice">TachoMaster status could not be checked. Vehicle positions remain available, but card-holder names may be missing.</p>}{fleet.data?.provider === 'Master data' && <p className="notice">Fleet status endpoint is unavailable, so this view is showing Master Data only.</p>}{fleet.data?.provider.includes('direct telemetry') && <p className="notice">The combined fleet endpoint failed, so this is raw RoadTech telemetry. Job links and TachoMaster card-holders are unavailable until the API/database issue is repaired.</p>}{fleet.data?.vehicleCount ? <FleetRollout fleet={fleet.data} /> : <div className="state"><strong>No active vehicles are in Master Data yet.</strong><span>Import the Transport Operations master workbook, then approve vehicle records in Staging Review.</span><Link to="/master-data">Import Master Data</Link><Link to="/staging">Open Staging Review</Link></div>}</> : <><div className="panel"><strong>{history.data?.recordCount || 0} stored tracking events</strong><span>History for {date}</span></div><div className="tracking-grid">{history.data?.records.map(record => <article className="vehicle-card" key={`${record.vehicleIdentifier}-${record.eventTimeUtc}`}><h2>{record.vehicleIdentifier}</h2><p>{record.isMoving ? 'Moving' : 'Stationary'} · {record.speedKph ?? 0} km/h</p><small>Recorded {formatDate(record.eventTimeUtc)}</small></article>)}</div></>}</State></section>; }
...
function MasterValidation({ issues }: { issues: string[] }) {
  const token = useAccessToken();
  const suggestions = useApi(useCallback(async () => api.masterDataSuggestions(await token()), [token]));
  const rules = suggestions.data?.suggestions || [];
  const combined = [...issues.map(message => ({ message, severity: 'warning' })), ...rules.slice(0, 8)];
  return <div className={combined.length ? 'master-validation warning' : 'master-validation'}><strong>{combined.length ? `${combined.length} validation and improvement item${combined.length === 1 ? '' : 's'}` : 'No obvious validation issues'}</strong>{suggestions.loading ? <span>Checking linked master-data rules…</span> : combined.length ? <ul>{combined.map((issue, index) => <li key={`${issue.message}-${index}`}>{issue.message}</li>)}</ul> : <span>Records loaded cleanly from the live cloud master data.</span>}{suggestions.error && <small>Background rule check unavailable; local checks are still shown.</small>}</div>;
}
function MasterTable<T>({ rows, columns, rowKey }: { rows: T[]; columns: Array<[string, (row: T) => unknown]>; rowKey: (row: T) => string }) {
  const exportRows = [columns.map(([label]) => label), ...rows.map(row => columns.map(([, value]) => normaliseTableValue(value(row))))];
  return <div className="master-table-wrap"><div className="table-toolbar"><span>{rows.length} record{rows.length === 1 ? '' : 's'}</span><button onClick={() => downloadCsv('slh-master-export', exportRows)} disabled={!rows.length}>Export CSV</button></div><table className="master-table"><thead><tr>{columns.map(([label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{rows.map(row => <tr key={rowKey(row)}>{columns.map(([label, value]) => <td key={label}>{normaliseTableValue(value(row))}</td>)}</tr>)}</tbody></table></div>;
}
function MasterFilter({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) { return <label className="master-filter">{label}<select value={value} onChange={event => onChange(event.target.value)}><option value="">All</option>{options.filter(Boolean).sort().map(option => <option key={option} value={option}>{option}</option>)}</select></label>; }

export function DriversMaster() {
  const token = useAccessToken(); const drivers = useApi(useCallback(async () => api.drivers(await token()), [token])); const [driverFilter, setDriverFilter] = useState('');
  const allRows = drivers.data || []; const rows = allRows.filter(row => !driverFilter || row.displayName === driverFilter || row.driverType === driverFilter || row.driverGroup === driverFilter);
  ...
  return <section><div className="title-row"><div><p className="eyebrow">Master data</p><h1>Drivers</h1></div><button onClick={() => void drivers.refresh()}>Refresh</button></div><p className="intro">One live row per driver. Sage and TachoMaster should sync into this page so employee numbers, tacho names, mobile numbers and planning priority stay together.</p><div className="master-filters"><MasterFilter label="Driver / group / type" value={driverFilter} onChange={setDriverFilter} options={allRows.flatMap(row => [row.displayName || '', row.driverType || '', row.driverGroup || ''])} /></div><MasterValidation issues={issues} /><MasterWorkbookImport mode="drivers" onApplied={drivers.refresh} /><State loading={drivers.loading} error={drivers.error} empty={!rows.length}><MasterTable rows={rows} rowKey={row => row.id} columns={[["Employee", row => row.employeeNumber], ["Driver", row => row.displayName], ["Tacho name", row => row.tachoName], ["Phone number", row => row.mobileNumber], ["Driver type", row => row.driverType], ["Driver group", row => row.driverGroup], ["Skills", row => row.skills], ["Active", row => row.active]]} /></State><DriverQuickAdd onSaved={drivers.refresh} /></section>;
}
...
export function FleetAssetsMaster() {
  const token = useAccessToken(); const vehicles = useApi(useCallback(async () => api.vehicles(await token()), [token])); const trailers = useApi(useCallback(async () => api.trailers(await token()), [token])); const fleetio = useApi(useCallback(async () => api.fleetioVehicleAlignment(await token()), [token])); const [vehicleFilter, setVehicleFilter] = useState(''); const [trailerFilter, setTrailerFilter] = useState('');
  const allVehicleRows = vehicles.data || []; const allTrailerRows = trailers.data || []; const vehicleRows = allVehicleRows.filter(row => !vehicleFilter || (vehicleFilter === 'Active' ? row.active : row.registration === vehicleFilter || row.fleetNumber === vehicleFilter || row.fleetioStatus === vehicleFilter)); const trailerRows = allTrailerRows.filter(row => !trailerFilter || row.trailerNumber === trailerFilter || row.type === trailerFilter);
  const issues = [...vehicleRows.flatMap(vehicle => [!vehicle.abbreviation ? `${vehicle.registration} missing abbreviation for RoadTech matching.` : '', !vehicle.fuelPinSecretName ? `${vehicle.registration} missing fuel PIN secret reference.` : '', !vehicle.fuelCardLastFour ? `${vehicle.registration} missing fuel card last four.` : ''].filter(Boolean)), ...trailerRows.flatMap(trailer => [!trailer.standardCapacity ? `Trailer ${trailer.trailerNumber} missing standard capacity.` : '', !trailer.euroCapacity ? `Trailer ${trailer.trailerNumber} missing Euro capacity.` : ''].filter(Boolean)), ...(fleetio.data?.records || []).flatMap(record => record.status === 'Matched' ? [] : [`Fleetio alignment: ${record.tmsRegistration || record.fleetioRegistration || record.fleetioName} is ${record.status}.`])];
  const refreshAll = () => { void vehicles.refresh(); void trailers.refresh(); void fleetio.refresh(); };
  return <section><div className="title-row"><div><p className="eyebrow">Master data</p><h1>Vehicles, fuel pins & trailers</h1></div><button onClick={refreshAll}>Refresh</button></div><p className="intro">This is the fleet reference list used for RoadTech matching, fuel-card texts, trailer capacity checks and Fleetio service/VOR alignment.</p><div className="master-filters"><MasterFilter label="Vehicle / status" value={vehicleFilter} onChange={setVehicleFilter} options={['Active', ...allVehicleRows.flatMap(row => [row.registration || '', row.fleetNumber || '', row.fleetioStatus || ''])]} /><MasterFilter label="Trailer / type" value={trailerFilter} onChange={setTrailerFilter} options={allTrailerRows.flatMap(row => [row.trailerNumber || '', row.type || ''])} /></div><MasterValidation issues={issues} /><MasterWorkbookImport mode="vehicles" onApplied={refreshAll} /><FleetioAlignmentPanel data={fleetio.data} loading={fleetio.loading} error={fleetio.error} onRefresh={fleetio.refresh} /><State loading={vehicles.loading || trailers.loading} error={vehicles.error || trailers.error}><h2 className="master-subtitle">Vehicles + fuel</h2><EditableVehicleTable rows={vehicleRows} onSaved={refreshAll} /><h2 className="master-subtitle">Trailers</h2><MasterTable rows={trailerRows} rowKey={row => row.id} columns={[["Trailer", row => row.trailerNumber], ["Type", row => row.type], ["Standard capacity", row => row.standardCapacity], ["Euro capacity", row => row.euroCapacity], ["Active", row => row.active]]} /></State><FleetQuickAdd onSaved={refreshAll} /></section>;
}
function EditableVehicleTable({ rows, onSaved }: { rows: Vehicle[]; onSaved: () => void }) {
  ...
  async function save(row: Vehicle) {
    ...
    if (!draft) return;
    try {
      await api.updateVehicle(row.id, { registration: draft.registration, fleetNumber: draft.fleetNumber, abbreviation: draft.abbreviation, transmission: draft.transmission, dvsCompliant: draft.dvsCompliant, fuelProvider: draft.fuelProvider, cabMobile: draft.cabMobile, fuelPin: draft.fuelPin, shellCard: draft.shellCard, bpRedCard: draft.bpRedCard, bpPlainCard: draft.bpPlainCard, notes: draft.notes, fuelPinSecretName: draft.fuelPinSecretName, fuelCardLastFour: draft.fuelCardLastFour, active: draft.active }, await token());
      cancel(row.id);
  ...
  return <div><div className="master-table-wrap"><table className="master-table editable-master-table"><thead><tr><th>Registration</th><th>Abbreviation</th><th>Transmission</th><th>DVS</th><th>Cab Mobile</th><th>Fuel PIN</th><th>Shell Card</th><th>BP Red Card</th><th>BP Plain Card</th><th>Notes</th><th>Active</th><th>Actions</th></tr></thead><tbody>{rows.map(row => { const draft = editing[row.id]; const current = draft || row; return <tr key={row.id}><td>{draft ? <input value={current.registration} onChange={event => update(row.id, 'registration', event.target.value)} /> : <button className="link-button" onClick={() => startEdit(row)}>{row.registration}</button>}</td><td>{draft ? <input value={current.abbreviation || ''} onChange={event => update(row.id, 'abbreviation', event.target.value)} /> : normaliseTableValue(row.abbreviation)}</td><td>{draft ? <input value={current.transmission || ''} onChange={event => update(row.id, 'transmission', event.target.value)} /> : normaliseTableValue(row.transmission)}</td><td>{draft ? <input type="checkbox" checked={Boolean(current.dvsCompliant)} onChange={event => update(row.id, 'dvsCompliant', event.target.checked)} /> : normaliseTableValue(row.dvsCompliant)}</td><td>{draft ? <input value={current.cabMobile || ''} onChange={event => update(row.id, 'cabMobile', event.target.value)} /> : normaliseTableValue(row.cabMobile)}</td><td>{draft ? <input value={current.fuelPin || ''} onChange={event => update(row.id, 'fuelPin', event.target.value)} /> : normaliseTableValue(row.fuelPin)}</td><td>{draft ? <input value={current.shellCard || ''} onChange={event => update(row.id, 'shellCard', event.target.value)} /> : normaliseTableValue(row.shellCard)}</td><td>{draft ? <input value={current.bpRedCard || ''} onChange={event => update(row.id, 'bpRedCard', event.target.value)} /> : normaliseTableValue(row.bpRedCard)}</td><td>{draft ? <input value={current.bpPlainCard || ''} onChange={event => update(row.id, 'bpPlainCard', event.target.value)} /> : normaliseTableValue(row.bpPlainCard)}</td><td>{draft ? <input value={current.notes || ''} onChange={event => update(row.id, 'notes', event.target.value)} /> : normaliseTableValue(row.notes)}</td><td>{draft ? <input type="checkbox" checked={current.active} onChange={event => update(row.id, 'active', event.target.checked)} /> : normaliseTableValue(row.active)}</td><td>{draft ? <span className="table-actions"><button className="primary" onClick={() => void save(row)}>Save</button><button onClick={() => cancel(row.id)}>Cancel</button></span> : <button onClick={() => startEdit(row)}>Edit</button>}</td></tr>; })}</tbody></table></div>{message && <p className="hint">{message}</p>}</div>;
}
function FleetioAlignmentPanel({ data, loading, error, onRefresh }: { data?: import('../lib/api').FleetioVehicleAlignment; loading: boolean; error?: string; onRefresh: () => void }) {
  if (loading) return <div className="panel fleetio-panel"><strong>Checking Fleetio vehicle alignment…</strong></div>;
  if (error) return <p className="notice inline-notice">Fleetio alignment could not refresh: {error}</p>;
  if (!data) return null;
  const due = (value?: string) => { if (!value) return '—'; const date = new Date(value); const days = Math.ceil((date.getTime() - Date.now()) / 86400000); const label = days < 0 ? 'Overdue' : days <= 30 ? 'Due soon' : 'Scheduled'; return `${label} · ${new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium' }).format(date)}`; };
  const complianceIssues = data.records.filter(row => row.fleetioVor || (row.pmiDueUtc && new Date(row.pmiDueUtc).getTime() < Date.now() + 30 * 86400000) || (row.motDueUtc && new Date(row.motDueUtc).getTime() < Date.now() + 30 * 86400000)).length;
  return <div className="panel fleetio-panel"><div><p className="eyebrow">Fleetio alignment by registration</p><h2>{data.connected ? `${data.matched} matched · ${complianceIssues} compliance item${complianceIssues === 1 ? '' : 's'} to watch` : 'Fleetio setup needed'}</h2><p>{data.message} Fleetio ID is retained only as metadata; registration is the match key.</p>{Boolean(data.missingSettings?.length) && <small>{data.missingSettings?.join(', ')}</small>}</div><button onClick={() => void onRefresh()}>Refresh Fleetio</button>{data.records.length > 0 && <div className="master-table-wrap fleetio-table"><table className="master-table"><thead><tr><th>Status</th><th>Registration</th><th>Fleetio status</th><th>VOR</th><th>PMI / service due</th><th>MOT due</th><th>Maintenance</th></tr></thead><tbody>{data.records.slice(0, 100).map((row, index) => <tr key={`${row.status}-${row.tmsRegistration || row.fleetioRegistration || index}`}><td><span className={`status ${row.status.toLowerCase()}`}>{row.status}</span></td><td>{row.tmsRegistration || row.fleetioRegistration || '—'}</td><td>{row.fleetioStatus || '—'}</td><td>{row.fleetioVor ? <span className="status failed">VOR</span> : 'No'}</td><td>{due(row.pmiDueUtc)}</td><td>{due(row.motDueUtc)}</td><td>{row.serviceStatus || '—'}</td></tr>)}</tbody></table></div>}</div>;
}
...
export function FuelMaster() { const token = useAccessToken(); const prices = useApi(useCallback(async () => api.fuelPrices(await token()), [token])); const rows = prices.data || []; const latest = rows[0]; const providers = new Set(rows.map(row => row.provider)); const issues = rows.length ? [] : ['No fuel prices loaded yet. Add this week’s provider prices below.']; return <section><div className="title-row"><div><p className="eyebrow">Master data</p><h1>Fuel price history</h1></div><button onClick={() => void prices.refresh()}>Refresh</button></div><p className="intro">Add weekly fuel prices here so pricing history stays in the cloud and can trend over time.</p><MasterValidation issues={issues} /><MasterWorkbookImport mode="fuel" onApplied={prices.refresh} /><div className="metrics"><Metric label="Latest week" value={latest?.weekCommencing || '—'} detail="Most recent upload" /><Metric label="Providers" value={String(providers.size)} detail="Tracked fuel suppliers" /><Metric label="History rows" value={String(rows.length)} detail="Stored in Azure SQL" /><Metric label="Pricing max" value={String(rows.find(row => row.isPricingMaximum)?.pricePencePerLitre ?? '—')} detail="Current max pence/litre" /></div><State loading={prices.loading} error={prices.error}><MasterTable rows={rows} rowKey={row => row.id} columns={[["Week commencing", row => row.weekCommencing], ["Provider", row => row.provider], ["Pence/litre", row => row.pricePencePerLitre], ["Pricing maximum", row => row.isPricingMaximum], ["Source", row => row.source], ["Notes", row => row.notes]]} /></State><FuelPriceForm onSaved={prices.refresh} /></section>; }
function FuelPriceForm({ onSaved }: { onSaved: () => void }) { const token = useAccessToken(); const [form, setForm] = useState({ weekCommencing: localDate(), provider: '', pricePencePerLitre: '', isPricingMaximum: false, notes: '' }); const [message, setMessage] = useState<string>(); const [saving, setSaving] = useState(false); async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); try { await api.saveFuelPrice({ weekCommencing: form.weekCommencing, provider: form.provider, pricePencePerLitre: Number(form.pricePencePerLitre), isPricingMaximum: form.isPricingMaximum, source: 'SLH TMS Web', notes: form.notes }, await token()); setMessage('Fuel price saved to cloud history.'); setForm(current => ({ ...current, provider: '', pricePencePerLitre: '', isPricingMaximum: false, notes: '' })); onSaved(); } catch (exception) { setMessage(exception instanceof Error ? exception.message : 'Fuel price could not be saved.'); } finally { setSaving(false); } } return <form className="quick-order master-edit-form" onSubmit={event => void submit(event)}><h2>Add weekly fuel price</h2><div className="field-grid"><label>Week commencing<input type="date" required value={form.weekCommencing} onChange={event => setForm(current => ({ ...current, weekCommencing: event.target.value }))} /></label><label>Provider<input required value={form.provider} onChange={event => setForm(current => ({ ...current, provider: event.target.value }))} /></label><label>Pence per litre<input required type="number" step="0.01" value={form.pricePencePerLitre} onChange={event => setForm(current => ({ ...current, pricePencePerLitre: event.target.value }))} /></label><label className="checkbox-label"><input type="checkbox" checked={form.isPricingMaximum} onChange={event => setForm(current => ({ ...current, isPricingMaximum: event.target.checked }))} /> Use as pricing maximum</label><label className="wide">Notes<input value={form.notes} onChange={event => setForm(current => ({ ...current, notes: event.target.value }))} /></label></div><button className="primary" disabled={saving}>{saving ? 'Saving…' : 'Save fuel price'}</button>{message && <p className="hint">{message}</p>}</form>; }
...
function MasterWorkbookImport({ mode = 'all', onApplied }: { mode?: MasterImportMode; onApplied?: () => void }) {
  ...
  function prepare(nextWorkbook: XLSX.WorkBook, nextSheet: string) {
    ...
    setSummary(`${mapped.records.length} ${scope} record${mapped.records.length === 1 ? '' : 's'} found across the workbook: ${summariseBatch(mapped.records) || 'nothing recognised'}. Review and edit the familiar tabs below before applying.`);
  ...
  function loadSheetPreview(nextWorkbook: XLSX.WorkBook, nextSheet: string) {
    ...
    if (!sheet) { setPreviewRows([]); return; }
    const rows = XLSX.utils.sheet_to_json<Array<unknown>>(sheet, { header: 1, defval: '', blankrows: true, raw: false });
    const width = Math.min(100, Math.max(1, ...rows.map(row => row.length)));
    setPreviewRows(rows.slice(0, 300).map(row => Array.from({ length: width }, (_, column) => text(row[column]))));
  }

  function editCell(rowIndex: number, columnIndex: number, value: string) {
    setPreviewRows(current => current.map((row, index) => index === rowIndex ? row.map((cell, column) => column === columnIndex ? value : cell) : row));
  }
  ...
  function commitCell(rowIndex: number, columnIndex: number, value: string) {
    ...
    else sheet[address] = { ...(sheet[address] || {}), t: 's', v: value, w: value };
    const range = sheet['!ref'] ? XLSX.utils.decode_range(sheet['!ref']) : { s: { r: 0, c: 0 }, e: { r: rowIndex, c: columnIndex } };
    range.e.r = Math.max(range.e.r, rowIndex); range.e.c = Math.max(range.e.c, columnIndex);
    sheet['!ref'] = XLSX.utils.encode_range(range);
    const nextWorkbook = { ...workbook, Sheets: { ...workbook.Sheets, [selectedSheet]: sheet } };
  ...
  async function selectWorkbook(file?: File) {
    if (!file) return;
    setSummary(undefined); setIssues([]); setRecords([]); setPreviewRows([]);
    try {
      const nextWorkbook = XLSX.read(await file.arrayBuffer(), { type: 'array', cellDates: true });
      const nextSheet = preferredSheet(nextWorkbook, mode) || nextWorkbook.SheetNames[0] || '';
      setWorkbook(nextWorkbook);
  ...
  return <div className="panel import-panel master-import"><h2>{title}</h2><p>{mode === 'all' ? 'Upload the Transport Operations master workbook. Its worksheets appear below like Excel so you can review and correct cells before writing the recognised rows to live Master Data.' : `Upload the master workbook. The familiar worksheet tabs remain visible, while this page only applies recognised ${mode} records.`}</p><input type="file" accept=".xlsx,.xls,.xlsm,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel" onChange={event => void selectWorkbook(event.target.files?.[0])} />{sheetNames.length > 0 && <><div className="excel-sheet-tabs" role="tablist" aria-label="Workbook sheets">{sheetNames.map(sheet => <button type="button" role="tab" aria-selected={selectedSheet === sheet} className={selectedSheet === sheet ? 'active' : ''} key={sheet} onClick={() => changeSheet(sheet)}>{sheet}</button>)}</div><div className="excel-grid-wrap"><table className="excel-grid"><thead><tr><th className="excel-corner" />{(previewRows[0] || []).map((_, column) => <th key={column}>{XLSX.utils.encode_col(column)}</th>)}</tr></thead><tbody>{previewRows.map((row, rowIndex) => <tr key={rowIndex}><th>{rowIndex + 1}</th>{row.map((cell, columnIndex) => <td key={columnIndex}><input aria-label={`${selectedSheet} ${XLSX.utils.encode_col(columnIndex)}${rowIndex + 1}`} value={cell} onChange={event => editCell(rowIndex, columnIndex, event.target.value)} onBlur={event => commitCell(rowIndex, columnIndex, event.target.value)} /></td>)}</tr>)}</tbody></table></div>{previewRows.length >= 300 && <p className="hint">Showing the first 300 rows of this worksheet. All rows remain included when Master Data is applied.</p>}</>}<p className="hint">Edit a cell and move out of it to re-check the workbook. Driver ordering remains full-time, casual, LTD, then agency.</p>{summary && <p className="notice inline-notice">{summary}</p>}{issues.length > 0 && <div className="import-issues"><strong>Import notes</strong><ul>{issues.slice(0, 12).map(issue => <li key={issue}>{issue}</li>)}</ul>{issues.length > 12 && <small>{issues.length - 12} more notes hidden.</small>}</div>}{records.length > 0 && <div className="excel-apply-bar"><span><strong>{records.length}</strong> recognised record{records.length === 1 ? '' : 's'} across the workbook</span><button className="primary" disabled={submitting} onClick={() => void submit()}>{submitting ? 'Applying...' : `Apply ${records.length} records to Master Data`}</button></div>}</div>;
}
...
function mapMasterWorkbook(workbook: XLSX.WorkBook): { records: StageBatchRequest[]; issues: string[] } {
  ...
  for (const row of vehicleRows) {
    ...
    const fuelPin = firstText(row, ['Fuel PIN', 'Fuel Pin', 'Fuel Card PIN', 'PIN']);
    add('vehicle', registration, { registration, fleetNumber: firstText(row, ['Fleet Number', 'Fleet No', 'FleetNumber']), fleetioId: firstText(row, ['Fleetio ID', 'FleetioId', 'Fleetio Vehicle ID', 'Fleetio VehicleId']), fleetioName: firstText(row, ['Fleetio Name', 'FleetioName']), fleetioStatus: firstText(row, ['Fleetio Status', 'FleetioStatus']), abbreviation: firstText(row, ['Abbreviation', 'Short Reg', 'Reg Last 3']) || registration.slice(-3), transmission: firstText(row, ['Transmission', 'Gearbox']), dvsCompliant: yesNo(read(row, 'DVS') || read(row, 'DVS Compliant')), fuelProvider: bestFuelProvider(row), cabMobile: normalisePhone(firstText(row, ['Cab Mobile', 'Cab Phone', 'Cab Phone Number', 'Cab Telephone', 'Mobile', 'Phone'])), fuelPin, shellCard: firstText(row, ['Shell Card', 'Shell']), bpRedCard: firstText(row, ['BP Red Card', 'BP Red']), bpPlainCard: firstText(row, ['BP Plain Card', 'BP Plain']), notes: firstText(row, ['Notes', 'Comments']), fuelPinSecretName: fuelPin ? `vehicle-${registration.toLowerCase()}-fuel-pin` : undefined, fuelCardLastFour: lastFour(read(row, 'BP Plain Card') || read(row, 'BP Red Card') || read(row, 'Shell Card')), active: true });
  }
...
function vehicleSheetRows(workbook: XLSX.WorkBook): SheetRow[] {
  ...
  const vehicleContextHeaders = new Set(['vehicleid', 'fleetnumber', 'fleetno', 'abbreviation', 'transmission', 'dvs', 'dvscompliant', 'cabmobile', 'cabphone', 'cabphonenumber', 'fuelpin', 'fuelcardpin', 'shellcard', 'bpredcard', 'bpplaincard', 'fuelprovider', 'fleetioid', 'fleetiovehicleid', 'fleetioname', 'fleetiostatus']);
  for (const candidate of candidateSheets) {
    const rows = XLSX.utils.sheet_to_json<Array<string | number | boolean | Date | undefined>>(workbook.Sheets[candidate], { header: 1, defval: '', blankrows: false });
    const headerIndex = rows.findIndex(row => {
...
function driverHeaderScore(row: Array<string | number | boolean | Date | undefined>) {
  ...
  const hasDriverContext = headers.some(header => ['tachoname', 'tachomastername', 'tacho', 'cardname', 'drivertype', 'employmenttype', 'drivergroup', 'mobile', 'mobilenumber', 'phonenumber'].includes(header));
