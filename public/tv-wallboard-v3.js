(function () {
  ...
  var pageIndex = 0;
  var state = {
    loads: [], assignments: [], progress: [], etas: [], timing: [],
    error: '', refreshedAt: null, successfulRefreshes: 0
  };
  ...
  function isCompleted(progress, timing) {
    ...
    var completed = Number(progress.completedStops || 0);
  ...
  function buildRows() {
    ...
    for (i = 0; i < state.loads.length; i += 1) {
      ...
      var timing = timings[String(load.id)] || null;
      if (/cancelled|completed/i.test(String(load.status || '')) || isCompleted(prog, timing)) { continue; }
      ...
      var finalLabel = timingEta
        ? (String(timing.finalEtaSource || '').toLowerCase() === 'geofence' ? 'LIVE FINAL ETA' : 'ESTIMATED FINAL ETA')
        : eta && eta.etaUtc
          ? (String(eta.source || '').toLowerCase() === 'live' ? 'LIVE FINAL ETA' : 'ESTIMATED FINAL ETA')
          : 'PLANNED FINAL';
      rows.push({
        id: load.id,
        load: load,
  ...
  function render() {
    ...
    if (coverage) {
      ...
      coverage.innerHTML = '<b>RUN VIEW</b><span>' + esc(rows.length + ' active runs') + '</span><span>' + esc(stopCount + ' physical stops') + '</span><span>Completed final destinations are removed automatically</span>';
    }
  ...
  function refresh() {
    ...
    function done(name, err, data, status) {
      if (err) {
        ...
        var nextEtas = data && data.records ? data.records : [];
        if (nextEtas && nextEtas.length) { state.etas = nextEtas; }
      }
      else if (name === 'timing') {
        var nextTiming = data && data.records ? data.records : [];
        if (nextTiming && nextTiming.length) { state.timing = nextTiming; }
      }
      pending -= 1;
      if (pending === 0) {
        ...
        state.error = coreErrors.length ? coreErrors.join(' ') : optionalErrors.length ? 'Live refresh delayed — retaining the last confirmed ETA and progress.' : '';
        state.refreshedAt = new Date();
        if (!coreErrors.length) { state.successfulRefreshes += 1; }
        render();
      }
    }
