...
async function installApi(page: Page, state: State) {
  await page.route('**/tms-api/api/v1/**', async route => {
    ...
    if (/^\/api\/v1\/runs\/.+\/allocation$/.test(path) && method === 'PUT') {
      const body = request.postDataJSON() as { driverId?: string | null; vehicleId?: string | null; trailerId?: string | null };
      ...
      return json(route, { ...canonicalRun(state), driverId: body.driverId, vehicleId: body.vehicleId, trailerId: body.trailerId });
    }
