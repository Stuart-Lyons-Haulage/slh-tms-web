...
function runReference(date: string) { return `RUN-${date}-1` }
...
async function installApi(page: Page, state: State) {
  await page.route('**/api/v1/**', async route => {
    ...
    if (path === '/api/v1/planner/available-orders' && method === 'GET') return json(route, [{
      id: orderId, customerName: 'Hall Hunter', collectionSite: 'Hall Hunter', deliverySite: 'Leyland', palletCount: 4,
      deliveryDate: state.planningDate, status: 'Approved', source: 'E2E'
    }]);
    if (path === '/api/v1/planner/loads' && method === 'POST') {
      state.runCreated = true; state.allocatedPallets = 4;
      return json(route, { id: runId, reference: runReference(state.planningDate), planningDate: state.planningDate, status: 'Planned' });
    }
    if (path === '/api/v1/planner/loads' && method === 'GET') return json(route, state.runCreated ? [{
      id: runId, reference: runReference(state.planningDate), planningDate: state.planningDate, status: state.geofenceStage === 3 ? 'Completed' : state.geofenceStage > 0 ? 'InProgress' : 'Planned',
      palletSpacesUsed: state.allocatedPallets, totalPalletSpaces: 26,
      driverId: state.driverAssigned ? driverId : undefined, vehicleId: state.vehicleAssigned ? vehicleId : undefined, trailerId: state.trailerAssigned ? trailerId : undefined,
      stops: [
        { id: collectionStopId, sequence: 1, name: 'Collect · Hall Hunter', address: 'Hall Hunter', plannedArrivalUtc: atOffset(30) },
        { id: deliveryStopId, sequence: 2, name: 'Deliver · Leyland', address: 'Leyland', plannedArrivalUtc: atOffset(80) }
      ]
    }] : []);
    ...
    if (path === '/api/v1/driver-dispatch' && method === 'GET') return json(route, {
      ...
      loads: state.runCreated ? [{ id: runId, reference: runReference(state.planningDate), rawReference: runReference(state.planningDate), planningDate: state.planningDate, status: state.geofenceStage === 3 ? 'Completed' : state.geofenceStage > 0 ? 'InProgress' : 'Planned', driverId: state.driverAssigned ? driverId : undefined, vehicleId: state.vehicleAssigned ? vehicleId : undefined, trailerId: state.trailerAssigned ? trailerId : undefined, southbound: false, stops: [
        { id: collectionStopId, sequence: 1, name: 'Collect · Hall Hunter', plannedArrivalUtc: atOffset(30) },
        { id: deliveryStopId, sequence: 2, name: 'Deliver · Leyland', plannedArrivalUtc: atOffset(80) }
      ] }] : []
    });
    if (path === '/api/v1/driver-dispatch-status' && method === 'GET') return json(route, { planningDate: state.planningDate, drivers: [{
      driverId, dispatchStatus: state.driverAssigned ? 'Awaiting Dispatch' : 'No Run', operationalStatus: state.geofenceStage === 3 ? 'Completed' : state.geofenceStage > 0 ? 'Working' : state.driverAssigned ? 'Awaiting Dispatch' : 'No Run', driverConfirmed: false,
